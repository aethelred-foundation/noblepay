...
- Contract deployment requires an HTTPS `RPC_URL` for `testnet` or `mainnet`; plaintext HTTP is permitted only when `CHAIN_ENV=devnet` for a local development chain. `http://54.165.44.130:8545` is explicitly prohibited for release deployment and runtime configuration.
