...
/**
 * @title MockISeal
 * @notice Test double for the Aethelred ISeal precompile (0x0900). The suite
 *         installs its runtime bytecode at the precompile address with
 ...
 *         (internal/evmhost/noblepay_test.go, a real-precompile test against a
 *         real seal keeper); this mock only exists to exercise the
 *         SealSettlementGate's own logic (purpose binding, permanence, live
 *         revocation, policy delegation).
 */
contract MockISeal is ISeal {
    ...
    function getSeal(string calldata sealId)
        external
        view
        returns (bytes32, bytes32, bytes32, int64, uint64, string memory, string memory purpose, uint8 status, string memory)
    {
        MockSeal storage s = _byId[sealId];
        require(s.exists, "seal not found");
        return (bytes32(0), bytes32(0), bytes32(0), 0, 0, "", s.purpose, s.active ? 2 : 3, "");
    }

    function getConfidentiality(string calldata)
        external
        pure
        returns (string memory, string memory, string memory, bytes memory, string memory, string memory, bool, bytes memory, string memory)
    {
        return ("", "", "", "", "", "", false, "", "");
    }
