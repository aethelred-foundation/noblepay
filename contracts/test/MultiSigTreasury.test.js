...
describe("MultiSigTreasury", function () {
  ...
  describe("Deployment", function () {
    ...
    it("should revert with fewer than 2 signers", async function () {
      ...
      await expect(
        Treasury.deploy(admin.address, [s1.address], 1, 1, 1, 1),
    ...
    it("should revert with invalid config (threshold > signers)", async function () {
      ...
      await expect(
        Treasury.deploy(admin.address, [s1.address, s2.address], 1, 2, 3, 3),
  ...
  describe("Proposal Lifecycle", function () {
    it("should create a proposal", async function () {
      const { treasury, usdc, signer1, recipient } =
    ...
    it("should revert execute before timelock", async function () {
      const { treasury, signer1, signer2, proposalId } =
        await proposalCreatedFixture();
      await treasury.connect(signer2).approveProposal(proposalId);
      await expect(
        treasury.connect(signer1).executeProposal(proposalId),
    ...
    it("should cancel a proposal", async function () {
      const { treasury, signer1, proposalId } = await proposalCreatedFixture();
      await expect(
        treasury.connect(signer1).cancelProposal(proposalId),
      ).to.emit(treasury, "ProposalCancelled");
    });

    it("should revert for non-signer", async function () {
      const { treasury, usdc, other, recipient } =
        await loadFixture(deployFixture);
      await expect(
        treasury
          .connect(other)
  ...
  describe("Budget Management", function () {
    ...
    it("should revert zero allocation", async function () {
      ...
      await expect(
        treasury
          .connect(budgetMgr)
          .createBudget("Empty", 0, 0, 0, 0, 0, periodEnd),
  ...
  describe("Recurring Payments", function () {
    it("should create a recurring payment", async function () {
      const { treasury, admin, signer1, recipient, usdc } =
        await loadFixture(deployFixture);
      await expect(
    ...
    it("should execute a due recurring payment", async function () {
      const { treasury, admin, signer1, recipient, usdc, other } =
        await loadFixture(deployFixture);
      ...
      await expect(
        treasury.connect(other).executeRecurringPayment(paymentId),
    ...
    it("should revoke a recurring payment", async function () {
      ...
      await expect(
        treasury.connect(signer1).revokeRecurringPayment(paymentId),
  ...
  describe("Delegation", function () {
    it("should delegate signing authority", async function () {
      ...
      await expect(
        treasury
          .connect(signer1)
          .delegateSigningAuthority(delegate.address, 86400 * 7),
    ...
    it("should revert delegation too long", async function () {
      ...
      await expect(
        treasury
          .connect(signer1)
          .delegateSigningAuthority(delegate.address, 86400 * 31),
    ...
    it("should revoke delegation", async function () {
      ...
      await treasury
        ...
        .delegateSigningAuthority(delegate.address, 86400 * 7);
      await expect(
        treasury.connect(signer1).revokeDelegation(delegate.address),
      ).to.emit(treasury, "DelegationRevoked");
    });

    it("delegate should be able to create proposal", async function () {
      const { treasury, usdc, signer1, delegate, recipient } =
        await loadFixture(deployFixture);
      await treasury
        .connect(signer1)
        .delegateSigningAuthority(delegate.address, 86400 * 7);
  ...
  describe("Admin", function () {
    it("should set supported token", async function () {
      const { treasury, admin, other } = await loadFixture(deployFixture);
      await expect(
        treasury.connect(admin).setSupportedToken(other.address, true),
