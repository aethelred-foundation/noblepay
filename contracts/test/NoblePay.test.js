...
describe("NoblePay", function () {
  ...
  describe("Deployment", function () {
    it("should set roles and fees correctly", async function () {
      const { noblepay, admin, treasury, ADMIN_ROLE } = await loadFixture(
        deployFixture
      );
    ...
    it("should revert with zero admin address", async function () {
      const NoblePay = await ethers.getContractFactory("NoblePay");
      await expect(
        NoblePay.deploy(ethers.ZeroAddress, ethers.ZeroAddress, 0, 0)
    ...
    it("should revert with zero treasury address", async function () {
      ...
      const NoblePay = await ethers.getContractFactory("NoblePay");
      await expect(
        NoblePay.deploy(admin.address, ethers.ZeroAddress, 0, 0)
      ).to.be.revertedWithCustomError(NoblePay, "ZeroAddress");
    });
    ...
    it("should revert with fee exceeding max", async function () {
      ...
      const NoblePay = await ethers.getContractFactory("NoblePay");
      await expect(
        NoblePay.deploy(admin.address, treasury.address, 0, 501)
  ...
  describe("Payment Initiation", function () {
    it("should initiate an ERC20 payment", async function () {
      const { noblepay, usdc, business1, recipient } = await loadFixture(
        deployFixture
      );
    ...
    it("should reject native payments", async function () {
      const { noblepay, business1, recipient } = await loadFixture(
        deployFixture
      );
    ...
    it("should revert for unregistered business", async function () {
      const { noblepay, usdc, other, recipient } = await loadFixture(
        deployFixture
      );
      const amount = ethers.parseUnits("100", 6);
      await expect(
    ...
    it("should revert for unsupported token", async function () {
      const { noblepay, business1, recipient, other } = await loadFixture(
        deployFixture
      );
      await expect(
        noblepay
    ...
    it("should reject native-token selection even with no msg.value", async function () {
      const { noblepay, business1, recipient } = await loadFixture(
        deployFixture
      );
      const amount = 1000n;
  ...
  describe("Settlement & Refund", function () {
    ...
    it("should revert settling a non-PASSED payment", async function () {
      ...
      await expect(
        noblepay.settlePayment(paymentId)
    ...
    it("should refund a blocked payment", async function () {
      ...
      await expect(noblepay.refundPayment(paymentId)).to.emit(
        noblepay,
        "PaymentRefunded"
      );
    });
    ...
    it("should revert cancel by non-sender", async function () {
      ...
      await expect(
        noblepay.connect(other).cancelPayment(paymentId)
  ...
  describe("Admin Functions", function () {
    it("should set supported token", async function () {
      ...
      const MockERC20 = await ethers.getContractFactory("MockERC20");
      const stablecoin = await MockERC20.deploy("Supported USD", "SUSD", 6);
      await expect(
        noblepay.connect(admin).setSupportedToken(stablecoin.target, true)
    ...
    it("should update fees", async function () {
      const { noblepay, admin } = await loadFixture(deployFixture);
      await expect(noblepay.connect(admin).setFees(100, 100)).to.emit(
        noblepay,
        "FeeUpdated"
      );
    });
    ...
    it("should revert fee update exceeding max", async function () {
      const { noblepay, admin } = await loadFixture(deployFixture);
      await expect(
        noblepay.connect(admin).setFees(100, 501)
      ).to.be.revertedWithCustomError(noblepay, "InvalidFee");
    });

    it("should update treasury", async function () {
      const { noblepay, admin, other } = await loadFixture(deployFixture);
      await expect(noblepay.connect(admin).setTreasury(other.address)).to.emit(
        noblepay,
        "TreasuryUpdated"
      );
    });
    ...
    it("should sync business", async function () {
      const { noblepay, admin, other } = await loadFixture(deployFixture);
      await expect(
        noblepay.connect(admin).syncBusiness(other.address, 1, true)
  ...
  describe("Batch Payments", function () {
    it("should initiate a batch of payments", async function () {
      const { noblepay, usdc, business1, recipient, other } = await loadFixture(
        deployFixture
      );
      const amount = ethers.parseUnits("100", 6);
      const transaction = await noblepay
        .connect(business1)
      ...
      const event = receipt.logs
        .map((log) => {
          try {
            return noblepay.interface.parseLog(log);
          } catch {
            return null;
          }
        })
        .find((entry) => entry?.name === "BatchProcessed");
      expect(event).to.not.equal(undefined);
      const batchId = event.args.batchId;
      const batch = await noblepay.batches(batchId);
      expect(batch.initiator).to.equal(business1.address);
      expect(batch.totalAmount).to.equal(amount * 2n);
      expect(batch.processed).to.equal(true);
      expect(await noblepay.getBatchPaymentIds(batchId)).to.have.length(2);
    });
