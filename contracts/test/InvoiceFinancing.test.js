...
describe("InvoiceFinancing", function () {
  async function deployFixture() {
    ...
    await invoiceF
      .connect(admin)
      .grantRole(await invoiceF.FACTOR_ROLE(), factor.address);
    await invoiceF
      .connect(admin)
      .grantRole(await invoiceF.CREDIT_ANALYST_ROLE(), analyst.address);
    await invoiceF
      .connect(admin)
      .grantRole(await invoiceF.ARBITER_ROLE(), arbiter.address);
  ...
  async function financedFixture() {
    ...
    const tx = await invoiceF
      .connect(factor)
      .financeInvoice(invoiceId, advance, 500); // 5% discount
    const receipt = await tx.wait();
    const event = receipt.logs.find(
  ...
  describe("Deployment", function () {
    it("should set admin and treasury", async function () {
      const { invoiceF, admin, treasuryAddr } =
    ...
    it("should revert with excessive fee", async function () {
      ...
      await expect(
        IF.deploy(a.address, t.address, 1001),
  ...
  describe("Invoice Creation", function () {
    it("should create an invoice", async function () {
      const { invoiceF, usdc, creditor, debtor } =
        await loadFixture(deployFixture);
    ...
    it("should revert for unsupported token", async function () {
      const { invoiceF, creditor, debtor, other } =
        await loadFixture(deployFixture);
      const maturity = BigInt(await time.latest()) + 86400n * 30n;
      await expect(
        invoiceF
  ...
  describe("Invoice Chaining", function () {
    it("should create a chained invoice", async function () {
      const { invoiceF, usdc, debtor, other, invoiceId } =
        await invoiceCreatedFixture();
    ...
    it("should revert if not debtor of parent", async function () {
      const { invoiceF, creditor, other, invoiceId } =
        await invoiceCreatedFixture();
      const maturity = BigInt(await time.latest()) + 86400n * 60n;
      await expect(
        invoiceF
  ...
  describe("Factoring", function () {
    it("should finance an invoice", async function () {
      ...
      await expect(
        invoiceF.connect(factor).financeInvoice(invoiceId, advance, 500),
    ...
    it("should revert exceeding face value", async function () {
      ...
      await expect(
        invoiceF
          .connect(factor)
          .financeInvoice(invoiceId, FACE_VALUE + 1n, 500),
    ...
    it("should revert excessive discount rate", async function () {
      const { invoiceF, factor, invoiceId } = await invoiceCreatedFixture();
      await expect(
        invoiceF.connect(factor).financeInvoice(invoiceId, FACE_VALUE, 5001),
  ...
  describe("Settlement", function () {
    it("should repay and settle invoice", async function () {
      ...
      await expect(
        invoiceF.connect(debtor).repayInvoice(invoiceId, FACE_VALUE),
      ).to.emit(invoiceF, "InvoiceSettled");
    });

    it("should distribute to factors on repayment", async function () {
      const { invoiceF, usdc, debtor, factor, invoiceId } =
        await financedFixture();
    ...
    it("should revert repay by non-debtor", async function () {
      ...
      await expect(
        invoiceF.connect(creditor).repayInvoice(invoiceId, FACE_VALUE),
      ).to.be.revertedWithCustomError(invoiceF, "NotDebtor");
    });
  });
  ...
  describe("Overdue", function () {
    it("should mark invoice overdue after maturity + grace", async function () {
      const { invoiceF, other, invoiceId, maturity } =
        await invoiceCreatedFixture();
      // maturity + 7 day grace + 1
      await time.increaseTo(maturity + 86400n * 7n + 1n);
      await expect(invoiceF.connect(other).markOverdue(invoiceId)).to.emit(
        invoiceF,
    ...
    it("should revert if not yet overdue", async function () {
      ...
      await expect(
        invoiceF.connect(other).markOverdue(invoiceId),
  ...
  describe("Disputes", function () {
    it("should initiate a dispute", async function () {
      ...
      await expect(
        invoiceF
          .connect(creditor)
          .initiateDispute(invoiceId, "goods not delivered"),
    ...
    it("should resolve a dispute", async function () {
      ...
      await expect(
        invoiceF
          .connect(arbiter)
          .resolveDispute(disputeId, 1, creditorAward, debtorRefund),
    ...
    it("should revert duplicate dispute", async function () {
      ...
      await expect(
        invoiceF.connect(creditor).initiateDispute(invoiceId, "issue2"),
    ...
    it("should revert dispute by non-party", async function () {
      const { invoiceF, other, invoiceId } = await invoiceCreatedFixture();
      await expect(
        invoiceF.connect(other).initiateDispute(invoiceId, "issue"),
      ).to.be.revertedWithCustomError(invoiceF, "NotInvoiceParty");
    });
  });
  ...
  describe("Collateral", function () {
    it("should deposit collateral", async function () {
      const { invoiceF, usdc, debtor, invoiceId } =
        await invoiceCreatedFixture();
      const amt = ethers.parseUnits("10000", 6);
      await expect(
        invoiceF.connect(debtor).depositCollateral(invoiceId, usdc.target, amt),
    ...
    it("should release collateral after settlement", async function () {
      ...
      await invoiceF
        .connect(debtor)
        .depositCollateral(invoiceId, usdc.target, amt);
      // Settle the invoice
      await invoiceF.connect(debtor).repayInvoice(invoiceId, FACE_VALUE);
      await expect(
        invoiceF.connect(debtor).releaseCollateral(invoiceId, debtor.address),
  ...
  describe("Credit Scoring", function () {
    it("should adjust credit score", async function () {
      ...
      await expect(
        invoiceF.connect(analyst).adjustCreditScore(creditor.address, 700),
    ...
    it("should revert invalid score", async function () {
      const { invoiceF, analyst, creditor } = await invoiceCreatedFixture();
      await expect(
        invoiceF.connect(analyst).adjustCreditScore(creditor.address, 299),
      ).to.be.revertedWithCustomError(invoiceF, "InvalidCreditScore");
      await expect(
        invoiceF.connect(analyst).adjustCreditScore(creditor.address, 851),
      ).to.be.revertedWithCustomError(invoiceF, "InvalidCreditScore");
    });

    it("should return suggested discount rate", async function () {
      const { invoiceF, other } = await loadFixture(deployFixture);
      expect(await invoiceF.getSuggestedDiscountRate(other.address)).to.equal(
        1500,
      ); // unknown
    });
  });
  ...
  describe("Cancel", function () {
    it("should cancel an unfunded invoice", async function () {
      const { invoiceF, creditor, invoiceId } = await invoiceCreatedFixture();
      await expect(invoiceF.connect(creditor).cancelInvoice(invoiceId)).to.emit(
        invoiceF,
        "InvoiceCancelled",
      );
    });
    ...
    it("should revert cancel by non-creditor", async function () {
      const { invoiceF, debtor, invoiceId } = await invoiceCreatedFixture();
      await expect(
        invoiceF.connect(debtor).cancelInvoice(invoiceId),
    ...
    it("should revert cancel of financed invoice", async function () {
      ...
      await expect(
        invoiceF.connect(creditor).cancelInvoice(invoiceId),
  ...
  describe("Batch Creation", function () {
    it("should batch create invoices", async function () {
      const { invoiceF, usdc, creditor, debtor, other } =
        await loadFixture(deployFixture);
      const maturity = BigInt(await time.latest()) + 86400n * 30n;
  ...
  describe("Admin", function () {
    it("should set supported token", async function () {
      ...
      await expect(
        invoiceF.connect(admin).setSupportedToken(other.address, true),
    ...
    it("should set protocol fee", async function () {
      ...
      await expect(invoiceF.connect(admin).setProtocolFee(200)).to.emit(
        invoiceF,
        "ProtocolFeeUpdated",
      );
    });
    ...
    it("should set business registry", async function () {
      const { invoiceF, admin, other } = await loadFixture(deployFixture);
      await expect(
        invoiceF.connect(admin).setBusinessRegistry(other.address),
