...
describe("BranchMax3", function () {
  ...
  describe("MultiSigTreasury — deep branch coverage", function () {
    async function deployMST() {
      const [admin, s1, s2, s3, s4, s5, budgetMgr, yieldMgr, recipient, other] =
      ...
      await mst
        .connect(admin)
        .grantRole(await mst.BUDGET_MANAGER_ROLE(), budgetMgr.address);
      await mst
        .connect(admin)
        .grantRole(await mst.YIELD_MANAGER_ROLE(), yieldMgr.address);
    ...
    it("reverts constructor with zero admin", async function () {
      ...
      await expect(
        MST.deploy(ethers.ZeroAddress, [a.address, b.address], 1, 1, 2, 2),
    ...
    it("reverts constructor with <2 signers", async function () {
      ...
      await expect(
        MST.deploy(admin.address, [a.address], 1, 1, 1, 1),
    ...
    it("reverts constructor with invalid thresholds (small=0)", async function () {
      ...
      await expect(
        MST.deploy(admin.address, [a.address, b.address], 0, 1, 2, 2),
    ...
    it("reverts constructor with invalid thresholds (large > signers)", async function () {
      ...
      const [admin, a, b] = await ethers.getSigners();
      await expect(
        MST.deploy(admin.address, [a.address, b.address], 1, 2, 3, 3),
      ).to.be.revertedWithCustomError(MST, "InvalidSignerConfig");
    });
    ...
    it("reverts constructor with zero signer address", async function () {
      ...
      const [admin, a] = await ethers.getSigners();
      await expect(
        MST.deploy(admin.address, [a.address, ethers.ZeroAddress], 1, 1, 2, 2),
      ).to.be.revertedWithCustomError(MST, "ZeroAddress");
    });
    ...
    it("reverts execute before timelock", async function () {
      ...
      await mst.connect(s2).approveProposal(pid);
      await expect(
        mst.connect(s1).executeProposal(pid),
    ...
    it("reverts double rejection", async function () {
      ...
      await mst.connect(s2).rejectProposal(pid);
      await expect(
        mst.connect(s2).rejectProposal(pid),
    ...
    it("reverts approve after expiry", async function () {
      ...
      await expect(
        mst.connect(s2).approveProposal(pid),
      ).to.be.revertedWithCustomError(mst, "ProposalExpiredError");
    });

    it("createBudget and proposal with budget", async function () {
      const { mst, usdc, admin, budgetMgr, s1, recipient } =
        await loadFixture(deployMST);
      ...
      const r = await tx.wait();
      const ev = r.logs.find(
        (l) => l.fragment && l.fragment.name === "BudgetCreated",
      );
    ...
    it("reverts budget with zero allocation", async function () {
      ...
      await expect(
        mst
          .connect(budgetMgr)
          .createBudget("Ops", 0, 0, 1000, 5000, 10000, periodEnd),
      ).to.be.revertedWithCustomError(mst, "ZeroAmount");
    });
    ...
    it("reverts budget with past period", async function () {
      ...
      await expect(
        mst
          .connect(budgetMgr)
          .createBudget("Ops", 0, 1000000, 1000, 5000, 10000, 1),
      ).to.be.reverted;
    });
    ...
    it("create and execute recurring payment (ERC20)", async function () {
      ...
      const tx = await createAuthorizedRecurringPayment(mst, admin, [
        recipient.address,
        usdc.target,
        amt,
        0,
      ...
      const r = await tx.wait();
      const ev = r.logs.find(
        (l) => l.fragment && l.fragment.name === "RecurringPaymentCreated",
      );
    ...
    it("reverts recurring payment not due", async function () {
      ...
      const r = await tx.wait();
      const rpId = r.logs.find(
        (l) => l.fragment && l.fragment.name === "RecurringPaymentCreated",
      ).args[0];
      await expect(
        mst.connect(s1).executeRecurringPayment(rpId),
    ...
    it("reverts delegation with zero delegate", async function () {
      ...
      await expect(
        mst.connect(s1).delegateSigningAuthority(ethers.ZeroAddress, 86400),
      ).to.be.revertedWithCustomError(mst, "ZeroAddress");
    });
    ...
    it("reverts delegation too long", async function () {
      ...
      await expect(
        mst.connect(s1).delegateSigningAuthority(other.address, 31 * 86400),
    ...
    it("reverts addSigner duplicate", async function () {
      ...
      await expect(
        mst.connect(admin).addSigner(s1.address),
    ...
    it("reverts removeSigner below threshold", async function () {
      ...
      await expect(
        mst.connect(admin).removeSigner(s1.address),
      ).to.be.revertedWithCustomError(mst, "MinimumSignersRequired");
    });
    ...
    it("reverts updateSignerConfig invalid", async function () {
      const { mst, admin } = await loadFixture(deployMST);
      await expect(
        mst.connect(admin).updateSignerConfig(0, 2, 3, 3),
      ).to.be.revertedWithCustomError(mst, "InvalidSignerConfig");
    });

    it("approveYieldProtocol and allocateToYield", async function () {
      const { mst, usdc, admin, yieldMgr, other } =
        await loadFixture(deployMST);
      await mst
        .connect(admin)
        .approveYieldProtocol(
          other.address,
          "Aave",
          ethers.parseUnits("1000000", 6),
        );
    ...
    it("reverts allocateToYield unapproved protocol", async function () {
      ...
      await expect(
        mst.connect(yieldMgr).allocateToYield(other.address, usdc.target, 1000),
    ...
    it("reverts allocateToYield exceeds max", async function () {
      ...
      await expect(
        mst.connect(yieldMgr).allocateToYield(other.address, usdc.target, 200),
    ...
    it("reverts allocateToYield zero amount", async function () {
      ...
      await mst
        .connect(admin)
        .approveYieldProtocol(other.address, "Aave", 10000);
      await expect(
        mst.connect(yieldMgr).allocateToYield(other.address, usdc.target, 0),
      ).to.be.revertedWithCustomError(mst, "ZeroAmount");
    });
    ...
    it("reverts setNoblePayContract zero", async function () {
      const { mst, admin } = await loadFixture(deployMST);
      await expect(
        mst.connect(admin).setNoblePayContract(ethers.ZeroAddress),
      ).to.be.revertedWithCustomError(mst, "ZeroAddress");
    });
    ...
    it("setSupportedToken zero reverts", async function () {
      const { mst, admin } = await loadFixture(deployMST);
      await expect(
        mst.connect(admin).setSupportedToken(ethers.ZeroAddress, true),
      ).to.be.revertedWithCustomError(mst, "ZeroAddress");
    });
    ...
    it("execute proposal with budget spending and daily/weekly/monthly tracking", async function () {
      const { mst, usdc, budgetMgr, s1, s2, recipient } =
        await loadFixture(deployMST);
      const periodEnd = BigInt(await time.latest()) + 90n * 86400n;
      const btx = await mst
        .connect(budgetMgr)
        .createBudget(
          ...
          ethers.parseUnits("500000", 6),
          ethers.parseUnits("900000", 6),
          periodEnd,
        );
      const br = await btx.wait();
      const budgetId = br.logs.find(
        (l) => l.fragment && l.fragment.name === "BudgetCreated",
      ).args[0];
  ...
  describe("NoblePay — deep branch coverage", function () {
    ...
    it("reverts constructor with zero admin", async function () {
      ...
      await expect(
        NP.deploy(ethers.ZeroAddress, t.address, 100, 50),
    ...
    it("reverts constructor with zero treasury", async function () {
      ...
      await expect(
        NP.deploy(a.address, ethers.ZeroAddress, 100, 50),
      ).to.be.revertedWithCustomError(NP, "ZeroAddress");
    });
    ...
    it("reverts constructor with excessive fee", async function () {
      ...
      await expect(
        NP.deploy(a.address, t.address, 100, 501),
    ...
    it("rejects initiatePayment with native token", async function () {
      ...
      ).to.be.revertedWithCustomError(np, "UnexpectedNativeValue");
    });

    it("rejects native payment without attached value", async function () {
      const { np, sender, recipient } = await loadFixture(deployNP);
      await expect(
        np
          .connect(sender)
      ...
      ).to.be.revertedWithCustomError(np, "NativePaymentsDisabled");
    });
    ...
    it("compliance: BLOCKED (sanctions fail)", async function () {
      const { np, usdc, sender, recipient, teeNode } =
        await loadFixture(deployNP);
      const pid = await initPayment(np, sender, recipient, usdc);
      await expect(
        np
          .connect(teeNode)
          .submitComplianceResult(pid, false, 50, true, ethers.ZeroHash, "0x"),
    ...
    it("compliance: FLAGGED (high risk score)", async function () {
      ...
      await expect(
        np
          .connect(teeNode)
          .submitComplianceResult(pid, true, 80, true, ethers.ZeroHash, "0x"),
    ...
    it("compliance: FLAGGED (travel rule fail)", async function () {
      ...
      await expect(
        np
          .connect(teeNode)
          .submitComplianceResult(pid, true, 30, false, ethers.ZeroHash, "0x"),
      ).to.emit(np, "PaymentFlagged");
    });
    ...
    it("compliance: PASSED then settle ERC20", async function () {
      ...
      await np
        .connect(teeNode)
        .submitComplianceResult(pid, true, 30, true, ethers.ZeroHash, "0x");
      await np.connect(sender).settlePayment(pid);
    });

    it("settle with zero fee", async function () {
      const { np, usdc, admin, sender, recipient, teeNode } =
        await loadFixture(deployNP);
    ...
    it("refund FLAGGED payment by compliance officer", async function () {
      const { np, usdc, sender, recipient, teeNode, officer } =
        await loadFixture(deployNP);
      const pid = await initPayment(np, sender, recipient, usdc);
      await np
        .connect(teeNode)
    ...
    it("reverts settle on non-PASSED payment", async function () {
      ...
      const pid = await initPayment(np, sender, recipient, usdc);
      await expect(
        np.connect(sender).settlePayment(pid),
    ...
    it("reverts compliance result with score > 100", async function () {
      ...
      await expect(
        np
          .connect(teeNode)
          .submitComplianceResult(pid, true, 101, true, ethers.ZeroHash, "0x"),
    ...
    it("reverts batch empty", async function () {
      ...
      await expect(
        np.connect(sender).initiatePaymentBatch([], [], [], [], []),
    ...
    it("reverts setFees excessive", async function () {
      const { np, admin } = await loadFixture(deployNP);
      await expect(
        np.connect(admin).setFees(0, 501),
      ).to.be.revertedWithCustomError(np, "InvalidFee");
    });
    ...
    it("reverts setTreasury zero", async function () {
      const { np, admin } = await loadFixture(deployNP);
      await expect(
        np.connect(admin).setTreasury(ethers.ZeroAddress),
      ).to.be.revertedWithCustomError(np, "ZeroAddress");
    });
    ...
    it("setSupportedToken zero reverts", async function () {
      const { np, admin } = await loadFixture(deployNP);
      await expect(
        np.connect(admin).setSupportedToken(ethers.ZeroAddress, true),
      ).to.be.revertedWithCustomError(np, "ZeroAddress");
    });
    ...
    it("PREMIUM tier has higher daily limit", async function () {
      const { np, registry, usdc, sender, recipient } =
        await loadFixture(deployNP);
      await registry.setBusiness(sender.address, true, 1); // PREMIUM
      await np
        .connect(sender)
        .initiatePayment(
          recipient.address,
    ...
    it("ENTERPRISE tier has highest daily limit", async function () {
      const { np, registry, usdc, sender, recipient } =
        await loadFixture(deployNP);
      await registry.setBusiness(sender.address, true, 2); // ENTERPRISE
      await np
        .connect(sender)
        .initiatePayment(
          recipient.address,
          ethers.parseUnits("501000", 6),
          usdc.target,
          PURPOSE,
          "0x414544",
        );
  ...
  describe("StreamingPayments — extra edges", function () {
    ...
    it("createBatchStreams empty reverts", async function () {
      ...
      await expect(
        sp.connect(sender).createBatchStreams([], token.target, [], 3600, 0),
      ).to.be.reverted;
    });
    ...
    it("createBatchStreams too many reverts", async function () {
      ...
      await expect(
        sp
          .connect(sender)
          .createBatchStreams(recipients, token.target, amounts, 3600, 0),
      ).to.be.reverted;
    });
    ...
    it("pauseStream already paused reverts", async function () {
      ...
      const tx = await sp
        .connect(sender)
        .createStream(recipient.address, token.target, amt, 3600, 0);
      const r = await tx.wait();
      const sid = r.logs.find(
  ...
  describe("InvoiceFinancing — extra edges", function () {
    ...
    it("reverts createInvoice with zero admin in constructor", async function () {
      ...
      const [, t] = await ethers.getSigners();
      await expect(
        IF.deploy(ethers.ZeroAddress, t.address, 100),
    ...
    it("depositCollateral with zero amount reverts", async function () {
      ...
      await expect(
        inv.connect(debtor).depositCollateral(invId, usdc.target, 0),
