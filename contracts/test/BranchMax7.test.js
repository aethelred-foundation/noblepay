...
describe("BranchMax7 — Modifier else-path branches", function () {
  ...
  describe("ComplianceOracle — whenNotPaused & onlyRole else-paths", function () {
    async function deployCO() {
      ...
      await co
        .connect(admin)
        .grantRole(await co.THRESHOLD_MANAGER_ROLE(), admin.address);
    ...
    it("paused: registerTEENode reverts", async function () {
      ...
      await expect(
        co
          .connect(tee2)
          .registerTEENode(key, pid, { value: ethers.parseEther("10") }),
      ).to.be.reverted;
    });
    ...
    it("submitScreeningResult with valid TEE node succeeds", async function () {
      ...
      await co
        .connect(tee1)
        .submitScreeningResult(subjectHash, resultHash, 30, true);
    });
    ...
    it("submitScreeningResult high risk score", async function () {
      ...
      await co
        .connect(tee1)
        .submitScreeningResult(subjectHash, resultHash, 90, false);
    });
    ...
    it("submitScreeningResult medium risk score", async function () {
      ...
      await co
        .connect(tee1)
        .submitScreeningResult(subjectHash, resultHash, 55, true);
    });
  ...
  describe("TravelRule — whenNotPaused & role paths", function () {
    async function deployTR() {
      ...
      await tr
        .connect(vasp1)
        .registerVASP(ethers.keccak256(ethers.toUtf8Bytes("inst1")), key1);
      await tr
        .connect(vasp2)
        .registerVASP(ethers.keccak256(ethers.toUtf8Bytes("inst2")), key2);
    ...
    it("paused: registerVASP reverts", async function () {
      ...
      await expect(tr.connect(other).registerVASP(ethers.ZeroHash, key)).to.be
        .reverted;
    });
    ...
    it("submitTravelRuleData succeeds from TEE node", async function () {
      ...
      await tr
        ...
        .submitTravelRuleData(
          ...
          ethers.keccak256(ethers.toUtf8Bytes("origName")),
          vasp1.address,
          ethers.keccak256(ethers.toUtf8Bytes("origInst")),
          ethers.keccak256(ethers.toUtf8Bytes("benefName")),
          vasp2.address,
  ...
  describe("FXHedgingVault — whenNotPaused & deeper paths", function () {
    ...
    it("paused: submitFXRate reverts", async function () {
      ...
      await fx.connect(admin).pause();
      await expect(
        fx.connect(oracle).submitFXRate(pairId, ethers.parseUnits("1.2", 8)),
      ).to.be.reverted;
    });
    ...
    it("submitFXRate from non-oracle reverts", async function () {
      ...
      await expect(
        fx.connect(other).submitFXRate(pairId, ethers.parseUnits("1.2", 8)),
      ).to.be.reverted;
    });
    ...
    it("addCurrencyPair duplicate reverts", async function () {
      ...
      await expect(
        fx
          .connect(admin)
          .addCurrencyPair("0x555344", "0x455552", 500, 300, 200),
      ).to.be.reverted;
    });
    ...
    it("settleForward after maturity", async function () {
      ...
      await fx
        .connect(oracle)
        .submitFXRate(pairId, ethers.parseUnits("1.15", 8));
      await usdc.mint(fx.target, ethers.parseUnits("100", 6));
      await expect(fx.connect(hedger).settleForward(posId)).to.emit(
        fx,
        "PositionSettled",
      );
    });
    ...
    it("exerciseOption (PUT) flow", async function () {
      ...
      await fx
        .connect(oracle)
        .submitFXRate(pairId, ethers.parseUnits("1.1", 8));
      await usdc.mint(fx.target, ethers.parseUnits("200", 6));
      await expect(fx.connect(hedger).exerciseOption(ev.args[0])).to.emit(
        fx,
        "OptionExercised",
      );
    });
  ...
  describe("LiquidityPool — whenNotPaused & deeper paths", function () {
    ...
    it("createPool with excessive fee reverts", async function () {
      ...
      await expect(
        pool
          .connect(admin)
          .createPool(token0.target, token1.target, 10000, 10, 500),
      ).to.be.reverted;
    });
    ...
    it("createPool with zero imbalance threshold reverts", async function () {
      const { pool, token0, token1, admin } = await loadFixture(deployLP);
      await expect(
        pool.connect(admin).createPool(token0.target, token1.target, 30, 10, 0),
      ).to.be.reverted;
    });
    ...
    it("paused: removeLiquidity reverts", async function () {
      ...
      const posId = lr.logs.find(
        (l) => l.fragment && l.fragment.name === "LiquidityAdded",
      ).args[0];
      await pool.connect(admin).pause();
      await expect(pool.connect(lp1).removeLiquidity(poolId, posId)).to.be
        .reverted;
    });

    it("addLiquidity by non-LP role reverts", async function () {
      const { pool, token0, token1, admin, other } =
    ...
    it("removeLiquidity by non-owner reverts", async function () {
      const { pool, token0, token1, admin, lp1, other } =
        await loadFixture(deployLP);
      const tx = await pool
        .connect(admin)
        .createPool(token0.target, token1.target, 30, 10, 500);
      ...
      await expect(pool.connect(other).removeLiquidity(poolId, posId)).to.be
        .reverted;
    });
    ...
    it("flashLoan on non-existent pool reverts", async function () {
      ...
      await expect(
        pool
          .connect(other)
          .flashLoan(fakePoolId, token0.target, ethers.parseEther("100"), "0x"),
      ).to.be.reverted;
    });

    it("setTreasury from non-admin reverts", async function () {
      const { pool, other } = await loadFixture(deployLP);
      await expect(pool.connect(other).setTreasury(other.address)).to.be
        .reverted;
    });
  ...
  describe("StreamingPayments — paused & deeper paths", function () {
    ...
    it("paused: createStream reverts", async function () {
      const { sp, admin, usdc, sender, recipient } =
        await loadFixture(deploySP);
      await sp.connect(admin).pause();
      await expect(
        sp
          .connect(sender)
          .createStream(recipient.address, usdc.target, 1000, 3600, 0),
      ).to.be.reverted;
    });
    ...
    it("cancel stream partially vested", async function () {
      ...
      const tx = await sp
        .connect(sender)
        .createStream(recipient.address, usdc.target, amount, 7200, 0);
      const r = await tx.wait();
      const streamId = r.logs.find(
        (l) => l.fragment && l.fragment.name === "StreamCreated",
      ).args[0];
  ...
  describe("CrossChainRouter — paused & deeper paths", function () {
    ...
    it("paused: registerRelay reverts", async function () {
      ...
      await expect(
        ccr.connect(relay1).registerRelay({ value: ethers.parseEther("5") }),
      ).to.be.reverted;
    });
  ...
  describe("PaymentChannels — more paused paths", function () {
    ...
    it("paused: fundChannel reverts", async function () {
      ...
      await expect(pc.connect(partyB).fundChannel(channelId, deposit)).to.be
        .reverted;
    });
    ...
    it("paused: claimHTLC remains available", async function () {
      const { pc, admin, usdc, partyA, partyB } = await loadFixture(deployPC);
      const deposit = ethers.parseUnits("10000", 6);
      const tx = await pc
        .connect(partyA)
    ...
    it("paused: refundHTLC remains available", async function () {
      const { pc, admin, usdc, partyA, partyB } = await loadFixture(deployPC);
      const deposit = ethers.parseUnits("10000", 6);
      const tx = await pc
        .connect(partyA)
    ...
    it("paused: batchOpenChannels reverts", async function () {
      ...
      await expect(
        pc
          .connect(partyA)
          .batchOpenChannels([partyB.address], usdc.target, [1000], 3600),
      ).to.be.reverted;
    });

    it("zero address is not KYC verified", async function () {
    ...
    it("setSupportedToken zero address reverts", async function () {
      ...
      await expect(
        pc.connect(admin).setSupportedToken(ethers.ZeroAddress, true),
      ).to.be.reverted;
    });
  });
  ...
  describe("NoblePay — paused paths", function () {
    ...
    it("paused: initiatePayment reverts", async function () {
      const { np, admin, usdc, sender, recipient } =
  ...
  describe("InvoiceFinancing — paused paths", function () {
    ...
    it("paused: createInvoice reverts", async function () {
      const { inv, admin, usdc, creditor, debtor } =
    ...
    it("paused: financeInvoice reverts", async function () {
      const { inv, admin, usdc, creditor, debtor, factor } =
        await loadFixture(deployIF);
      const now = await time.latest();
      const tx = await inv
        .connect(creditor)
      ...
      await expect(
        inv
          .connect(factor)
          .financeInvoice(invoiceId, ethers.parseUnits("5000", 6), 500),
      ).to.be.reverted;
    });
