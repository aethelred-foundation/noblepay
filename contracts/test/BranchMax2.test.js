...
describe("BranchMax2", function () {
  ...
  describe("PaymentChannels — deep branch coverage", function () {
    async function deployPC() {
      const [admin, treasury, partyA, partyB, other] =
      ...
      await configureMockBusinessRegistry(pc, admin, [partyA, partyB, other]);
      const amt = ethers.parseUnits("10000000", 6);
      for (const s of [partyA, partyB, other]) {
    ...
    async function openChannel(pc, token, partyA, partyB) {
      ...
      const tx = await pc
        .connect(partyA)
        .openChannel(partyB.address, token.target, deposit, 86400);
    ...
    async function openAndFundChannel(pc, token, partyA, partyB) {
      const { channelId, deposit } = await openChannel(
        pc,
        token,
        partyA,
        partyB,
    ...
    it("reverts constructor with zero admin", async function () {
      ...
      const PC = await ethers.getContractFactory("PaymentChannels");
      await expect(PC.deploy(ethers.ZeroAddress, t.address, 100)).to.be
        .reverted;
    });
    ...
    it("reverts constructor with zero treasury", async function () {
      ...
      const PC = await ethers.getContractFactory("PaymentChannels");
      await expect(PC.deploy(a.address, ethers.ZeroAddress, 100)).to.be
        .reverted;
    });
    ...
    it("partyA top-up on ACTIVE channel", async function () {
      ...
      const { channelId, deposit } = await openAndFundChannel(
        pc,
        token,
        partyA,
        partyB,
    ...
    it("partyB initiates unilateral close", async function () {
      ...
      await pc
        .connect(partyB)
        .initiateUnilateralClose(channelId, balA, balB, nonce, sigA);
    });
    ...
    it("settle with protocolFeeBps == 0", async function () {
      ...
      await configureMockBusinessRegistry(pc2, admin, [partyA, partyB]);
      ...
      await pc2
        .connect(partyA)
        .cooperativeClose(cid, deposit, deposit, 1n, sigA, sigB);
    });
    ...
    it("settle with payA == 0", async function () {
      ...
      await pc
        .connect(partyA)
        .cooperativeClose(channelId, 0n, totalDep, 1n, sigA, sigB);
    });
    ...
    it("batchOpenChannels with zero deposit reverts", async function () {
      ...
      await expect(
        pc
          .connect(partyA)
          .batchOpenChannels([partyB.address], token.target, [0], 86400),
      ).to.be.reverted;
    });
    ...
    it("finalize close after challenge period", async function () {
      ...
      await pc
        .connect(partyA)
        .initiateUnilateralClose(channelId, deposit, deposit, 1n, sigB);
  ...
  describe("FXHedgingVault — deep branch coverage", function () {
    ...
    async function deployFX() {
      const [admin, oracle, riskMgr, liquidator, hedger, other, treasury] =
        await ethers.getSigners();
      const MockERC20 = await ethers.getContractFactory("MockERC20");
      ...
      await vault
        .connect(admin)
        .grantRole(await vault.ORACLE_ROLE(), oracle.address);
      await vault
        .connect(admin)
        .grantRole(await vault.RISK_MANAGER_ROLE(), riskMgr.address);
      await vault
        .connect(admin)
        .grantRole(await vault.LIQUIDATOR_ROLE(), liquidator.address);
    ...
    it("reverts constructor with zero admin", async function () {
      const [, t] = await ethers.getSigners();
      const FX = await ethers.getContractFactory("FXHedgingVault");
      await expect(FX.deploy(ethers.ZeroAddress, t.address, 100)).to.be
        .reverted;
    });
    ...
    it("reverts constructor with zero treasury", async function () {
      const [a] = await ethers.getSigners();
      const FX = await ethers.getContractFactory("FXHedgingVault");
      await expect(FX.deploy(a.address, ethers.ZeroAddress, 100)).to.be
        .reverted;
    });
    ...
    it("submitFXRate non-existent pair reverts", async function () {
      ...
      await expect(
        vault.connect(oracle).submitFXRate(fakePair, 100000),
    ...
    it("batchSubmitFXRates with zero rate reverts", async function () {
      const { vault, oracle, pairId } = await loadFixture(deployFX);
      await expect(
        vault.connect(oracle).batchSubmitFXRates([pairId], [0]),
      ).to.be.revertedWithCustomError(vault, "InvalidRate");
    });
    ...
    it("batchSubmitFXRates with non-existent pair reverts", async function () {
      ...
      const fakePair = ethers.keccak256(ethers.toUtf8Bytes("XXX/YYY"));
      await expect(
        vault.connect(oracle).batchSubmitFXRates([fakePair], [100000]),
      ).to.be.revertedWithCustomError(vault, "PairNotFound");
    });
    ...
    it("createForward with zero notional reverts", async function () {
      ...
      await expect(
        vault
          .connect(hedger)
          .createForward(pairId, 0, maturity, usdc.target, 1000),
      ).to.be.reverted;
    });
    ...
    it("createForward with unsupported collateral reverts", async function () {
      ...
      await expect(
        vault
          .connect(hedger)
          .createForward(pairId, 1000000n, maturity, ethers.ZeroAddress, 1000),
      ).to.be.reverted;
    });
    ...
    it("settleForward with high fee consuming all settlement", async function () {
      const { vault, usdc, oracle, admin, hedger, pairId } =
        await loadFixture(deployFX);
    ...
    it("exerciseOption with fee == 0", async function () {
      ...
      await vault.connect(oracle).submitFXRate(pairId, 400000000n);
      await expect(vault.connect(hedger).exerciseOption(ev.args[0])).to.emit(
        vault,
        "OptionExercised",
      );
    });
    ...
    it("expireOption after maturity", async function () {
      ...
      expect(ev).to.not.equal(undefined);
      await time.increase(31 * 86400);
      await expect(vault.connect(hedger).expireOption(ev.args[0]))
        .to.emit(vault, "OptionExpired")
        .withArgs(ev.args[0]);
    });

    it("liquidatePosition on underwater forward", async function () {
      const { vault, usdc, oracle, liquidator, hedger, pairId } =
        await loadFixture(deployFX);
      const { positionId } = await createForward(vault, hedger, pairId, usdc);
      // Crash rate
      await vault.connect(oracle).submitFXRate(pairId, 100000000n);
      await expect(
        vault.connect(liquidator).liquidatePosition(positionId),
    ...
    it("emergencyUnwind by admin", async function () {
      const { vault, usdc, admin, hedger, pairId } =
        await loadFixture(deployFX);
      const { positionId } = await createForward(vault, hedger, pairId, usdc);
    ...
    it("assessHedgeEffectiveness with zero change", async function () {
      const { vault, usdc, riskMgr, hedger, pairId } =
        await loadFixture(deployFX);
      const { positionId } = await createForward(vault, hedger, pairId, usdc);
      await expect(
        vault.connect(riskMgr).assessHedgeEffectiveness(positionId, 0),
      )
        .to.emit(vault, "HedgeEffectivenessAssessed")
        .withArgs(positionId, 0, false);
    });
  });
  ...
  describe("TravelRule — deep branch coverage", function () {
    ...
    it("verify then share then acknowledge", async function () {
      ...
      const shareEv = shareR.logs.find(
    ...
    it("reject travel rule data", async function () {
      ...
      await tr
        .connect(teeNode)
        .rejectTravelRuleData(ev.args[0], "invalid data");
    });
    ...
    it("acknowledgement after deadline reverts", async function () {
      ...
      await expect(tr.connect(vasp2).acknowledgeTravelRuleData(shareEv.args[1]))
        .to.be.reverted;
    });
    ...
    it("requiresFullTravelRuleData checks threshold", async function () {
      ...
      expect(
        await tr.requiresFullTravelRuleData(ethers.parseUnits("999", 6)),
      ).to.equal(false);
      expect(
        await tr.requiresFullTravelRuleData(ethers.parseUnits("1000", 6)),
      ).to.equal(true);
    });
  ...
  describe("BusinessRegistry — deep branch coverage", function () {
    ...
    it("registerBusiness with valid UAE license", async function () {
      ...
      await br
        .connect(biz1)
        .registerBusiness("ABC123", "Test Corp", 0, officer.address);
    });
    ...
    it("registerBusiness duplicate reverts", async function () {
      ...
      await br
        .connect(biz1)
        .registerBusiness("ABC123", "Test", 0, officer.address);
      await expect(
        br
          .connect(biz1)
          .registerBusiness("DEF456", "Test2", 0, officer.address),
      ).to.be.reverted;
    });
    ...
    it("registerBusiness duplicate license reverts", async function () {
      ...
      await expect(
        br
          .connect(biz2)
          .registerBusiness("ABC123", "Test2", 0, officer.address),
      ).to.be.reverted;
    });
    ...
    it("registerBusiness with zero officer reverts", async function () {
      ...
      await expect(
        br
          .connect(biz1)
          .registerBusiness("ABC123", "Test", 0, ethers.ZeroAddress),
      ).to.be.reverted;
    });
    ...
    it("registerBusiness with short license reverts", async function () {
      const { br, biz1, officer } = await loadFixture(deployBR);
      await expect(
        br.connect(biz1).registerBusiness("ABC", "Test", 0, officer.address),
      ).to.be.reverted;
    });
    ...
    it("registerBusiness with empty name reverts", async function () {
      const { br, biz1, officer } = await loadFixture(deployBR);
      await expect(
        br.connect(biz1).registerBusiness("ABC123", "", 0, officer.address),
      ).to.be.reverted;
    });
    ...
    it("registerBusiness INTERNATIONAL (no char validation)", async function () {
      ...
      await br
        .connect(biz1)
        .registerBusiness("INT@#!", "Test", 1, officer.address);
    });
    ...
    it("upgradeTier already at max reverts", async function () {
      ...
      await expect(br.connect(admin).upgradeTier(biz1.address, 2)).to.be
        .reverted;
    });
    ...
    it("upgradeTier downgrade reverts", async function () {
      ...
      await br.connect(admin).verifyBusiness(biz1.address);
      await br.connect(admin).upgradeTier(biz1.address, 1); // PREMIUM
      await expect(br.connect(admin).upgradeTier(biz1.address, 0)).to.be
        .reverted; // STANDARD
    });
  ...
  describe("AIComplianceModule — deep branch coverage", function () {
    async function deployAI() {
      const [admin, operator, officer, appellant, other] =
        await ethers.getSigners();
    ...
    it("recordDecision auto-escalates low confidence", async function () {
      ...
      const tx = await ai
        .connect(admin)
        .recordDecision(sub, modelId, 0, 30, sub, sub);
      const r = await tx.wait();
      const ev = r.logs.find(
    ...
    it("recordDecision confidence > 100 reverts", async function () {
      ...
      await expect(
        ai.connect(admin).recordDecision(sub, modelId, 0, 101, sub, sub),
      ).to.be.reverted;
    });
    ...
    it("recordDecision with non-existent model reverts", async function () {
      ...
      await expect(
        ai.connect(admin).recordDecision(sub, fakeModel, 0, 80, sub, sub),
      ).to.be.reverted;
    });
    ...
    it("fileAppeal and resolve as overturned", async function () {
      ...
      const dtx = await ai
        .connect(admin)
        .recordDecision(sub, modelId, 2, 80, sub, sub); // REJECTED
    ...
    it("fileAppeal and resolve as upheld", async function () {
      ...
      const dtx = await ai
        .connect(admin)
        .recordDecision(sub, modelId, 2, 80, sub, sub);
      const dr = await dtx.wait();
      const dev = dr.logs.find(
        (l) => l.fragment && l.fragment.name === "DecisionRecorded",
      );
    ...
    it("fileAppeal after window reverts", async function () {
      ...
      const dtx = await ai
        .connect(admin)
        .recordDecision(sub, modelId, 0, 80, sub, sub);
      const dr = await dtx.wait();
      const dev = dr.logs.find(
        (l) => l.fragment && l.fragment.name === "DecisionRecorded",
      );
      await time.increase(31 * 86400);
      await expect(ai.connect(admin).fileAppeal(dev.args[0], sub)).to.be
        .reverted;
    });
    ...
    it("overrideDecision same outcome reverts", async function () {
      ...
      const dtx = await ai
        .connect(admin)
        .recordDecision(sub, modelId, 0, 80, sub, sub); // APPROVED
      const dr = await dtx.wait();
      const dev = dr.logs.find(
        (l) => l.fragment && l.fragment.name === "DecisionRecorded",
      );
      await expect(ai.connect(admin).overrideDecision(dev.args[0], 0, sub)).to
        .be.reverted; // same
    });
    ...
    it("overrideDecision already overridden reverts", async function () {
      ...
      await expect(ai.connect(admin).overrideDecision(dev.args[0], 1, sub)).to
        .be.reverted;
    });
    ...
    it("setEscalationThreshold > 100 reverts", async function () {
      const { ai, admin } = await loadFixture(deployAI);
      await expect(ai.connect(admin).setEscalationThreshold(101)).to.be
        .reverted;
    });
  ...
  describe("LiquidityPool — deep branch coverage", function () {
    async function deployLP() {
      const [admin, treasury, provider, trader, other] =
        await ethers.getSigners();
      const MockERC20 = await ethers.getContractFactory("MockERC20");
      ...
      await lp
        .connect(admin)
        .grantRole(await lp.LIQUIDITY_PROVIDER_ROLE(), provider.address);
    ...
    it("createPool and addLiquidity", async function () {
      ...
      const tx = await lp
        .connect(admin)
        .createPool(t0.target, t1.target, 30, 10, 8000);
      const r = await tx.wait();
      const ev = r.logs.find(
    ...
    it("createPool duplicate reverts", async function () {
      ...
      await expect(
        lp.connect(admin).createPool(t0.target, t1.target, 30, 10, 8000),
      ).to.be.reverted;
    });
    ...
    it("createPool excessive fee reverts", async function () {
      const { lp, t0, t1, admin } = await loadFixture(deployLP);
      await expect(
        lp.connect(admin).createPool(t0.target, t1.target, 101, 10, 8000),
      ).to.be.reverted;
    });
    ...
    it("createPool excessive flash fee reverts", async function () {
      const { lp, t0, t1, admin } = await loadFixture(deployLP);
      await expect(
        lp.connect(admin).createPool(t0.target, t1.target, 30, 51, 8000),
      ).to.be.reverted;
    });
    ...
    it("createPool invalid imbalance threshold reverts", async function () {
      const { lp, t0, t1, admin } = await loadFixture(deployLP);
      await expect(
        lp.connect(admin).createPool(t0.target, t1.target, 30, 10, 0),
      ).to.be.reverted;
      await expect(
        lp.connect(admin).createPool(t0.target, t1.target, 30, 10, 9501),
      ).to.be.reverted;
    });
    ...
    it("addLiquidity with zero amounts reverts", async function () {
      ...
      await expect(
        lp.connect(provider).addLiquidity(ev.args[0], 0, 0, -100, 100),
      ).to.be.reverted;
    });
    ...
    it("addLiquidity with invalid tick range reverts", async function () {
      ...
      // tickLower >= tickUpper
      await expect(
        lp.connect(provider).addLiquidity(ev.args[0], amt, amt, 100, 100),
      ).to.be.reverted;
    });
    ...
    it("addLiquidity with misaligned tick reverts", async function () {
      ...
      const amt = ethers.parseEther("10000");
      await expect(
        lp.connect(provider).addLiquidity(ev.args[0], amt, amt, -7, 13),
      ).to.be.reverted;
    });
    ...
    it("removeLiquidity and harvestFees", async function () {
      ...
      const poolId = ev.args[0];
      const amt = ethers.parseEther("10000");
      const addTx = await lp
        .connect(provider)
        .addLiquidity(poolId, amt, amt, -100, 100);
      const addR = await addTx.wait();
      const addEv = addR.logs.find(
    ...
    it("removeLiquidity not owner reverts", async function () {
      const { lp, t0, t1, admin, provider, other } =
        await loadFixture(deployLP);
      const tx = await lp
        .connect(admin)
        .createPool(t0.target, t1.target, 30, 10, 8000);
      const r = await tx.wait();
      ...
      const addTx = await lp
        .connect(provider)
        .addLiquidity(ev.args[0], amt, amt, -100, 100);
      const addR = await addTx.wait();
      const addEv = addR.logs.find(
        (l) => l.fragment && l.fragment.name === "LiquidityAdded",
      );
      await expect(lp.connect(other).removeLiquidity(ev.args[0], addEv.args[0]))
        .to.be.reverted;
    });
    ...
    it("updateCircuitBreaker invalid threshold reverts", async function () {
      ...
      await expect(lp.connect(admin).updateCircuitBreaker(ev.args[0], 0, 3600))
        .to.be.reverted;
    });
