...
describe("BranchMax5", function () {
  ...
  describe("InvoiceFinancing — guard/OR branches", function () {
    ...
    it("financeInvoice with non-existent invoice reverts", async function () {
      ...
      await expect(inv.connect(factor).financeInvoice(FAKE_ID, 1000, 500)).to.be
        .reverted;
    });
    ...
    it("repayInvoice with non-existent invoice reverts", async function () {
      ...
      await expect(inv.connect(debtor).repayInvoice(FAKE_ID, 1000)).to.be
        .reverted;
    });
    ...
    it("initiateDispute with non-existent invoice reverts", async function () {
      const { inv, debtor } = await loadFixture(deployIF);
      await expect(inv.connect(debtor).initiateDispute(FAKE_ID, "test")).to.be
        .reverted;
    });
    ...
    it("resolveDispute with non-existent dispute reverts", async function () {
      ...
      await expect(inv.connect(arbiter).resolveDispute(FAKE_ID, 1, 0, 0)).to.be
        .reverted;
    });
    ...
    it("depositCollateral with non-existent invoice reverts", async function () {
      ...
      await expect(
        inv.connect(debtor).depositCollateral(FAKE_ID, usdc.target, 1000),
      ).to.be.reverted;
    });
    ...
    it("releaseCollateral with non-existent invoice reverts", async function () {
      const { inv, debtor } = await loadFixture(deployIF);
      await expect(
        inv.connect(debtor).releaseCollateral(FAKE_ID, debtor.address),
      ).to.be.reverted;
    });
    ...
    it("depositCollateral with unsupported token reverts", async function () {
      const { inv, usdc, creditor, debtor, other } =
      ...
      await expect(
        inv.connect(debtor).depositCollateral(invId, other.address, 1000),
      ).to.be.reverted;
    });
    ...
    it("releaseCollateral already released reverts", async function () {
      ...
      await expect(inv.connect(debtor).releaseCollateral(invId, debtor.address))
        .to.be.reverted;
    });
    ...
    it("resolveDispute with PENDING outcome reverts", async function () {
      ...
      await expect(inv.connect(arbiter).resolveDispute(dev.args[0], 0, 0, 0)).to
        .be.reverted;
    });
    ...
    it("resolve already-resolved dispute reverts", async function () {
      ...
      // Try again
      await expect(inv.connect(arbiter).resolveDispute(dev.args[0], 1, 0, 0)).to
        .be.reverted;
    });
  });
  ...
  describe("MultiSigTreasury — guard branches", function () {
    ...
    it("revokeRecurringPayment non-existent reverts", async function () {
      const { mst, s1 } = await loadFixture(deployMST);
      await expect(mst.connect(s1).revokeRecurringPayment(FAKE_ID)).to.be
        .reverted;
    });

    it("executeRecurringPayment non-existent reverts", async function () {
      const { mst, s1 } = await loadFixture(deployMST);
      await expect(mst.connect(s1).executeRecurringPayment(FAKE_ID)).to.be
        .reverted;
    });
    ...
    it("approveYieldProtocol with zero address reverts", async function () {
      ...
      await expect(
        mst
          .connect(admin)
          .approveYieldProtocol(ethers.ZeroAddress, "test", 1000),
    ...
    it("removeSigner non-signer reverts", async function () {
      ...
      await expect(
        mst.connect(admin).removeSigner(other.address),
    ...
    it("addSigner zero address reverts", async function () {
      const { mst, admin } = await loadFixture(deployMST);
      await expect(
        mst.connect(admin).addSigner(ethers.ZeroAddress),
      ).to.be.revertedWithCustomError(mst, "ZeroAddress");
    });
  ...
  describe("CrossChainRouter — guard branches", function () {
    async function deployCCR() {
      const [admin, relay1, sender, treasury, other] =
        await ethers.getSigners();
      const MockERC20 = await ethers.getContractFactory("MockERC20");
    ...
    it("submitRelayProof non-existent transfer reverts", async function () {
      ...
      await expect(ccr.connect(relay1).submitRelayProof(FAKE_ID, destTx, "0x"))
        .to.be.reverted;
    });
    ...
    it("markTransferFailed non-existent reverts", async function () {
      const { ccr, admin } = await loadFixture(deployCCR);
      await expect(ccr.connect(admin).markTransferFailed(FAKE_ID, "fail")).to.be
        .reverted;
    });
  ...
  describe("LiquidityPool — guard branches", function () {
    it("removeLiquidity non-active position reverts", async function () {
      ...
      let token0 = t0,
        token1 = t1;
      if (BigInt(t0.target) > BigInt(t1.target)) {
        token0 = t1;
        token1 = t0;
      }
      ...
      await pool
        .connect(admin)
        .grantRole(await pool.POOL_ADMIN_ROLE(), admin.address);
      await pool
        .connect(admin)
      ...
      await expect(pool.connect(lp1).removeLiquidity(poolId, fakePos)).to.be
        .reverted;
    });
  });
});
