...
describe("BranchMax4", function () {
  ...
  describe("InvoiceFinancing — remaining branches", function () {
    ...
    async function createAndFinanceInvoice(
      inv,
      creditor,
      debtor,
      factor,
    ...
    it("getSuggestedDiscountRate with known credit profile", async function () {
      const { inv, usdc, creditor, debtor, factor, analyst } =
        await loadFixture(deployIF);
      // Create multiple invoices and repay to build credit score
      const invId = await createAndFinanceInvoice(
        inv,
        creditor,
        debtor,
        factor,
    ...
    it("releaseCollateral on RESOLVED invoice", async function () {
      ...
      await inv
        .connect(debtor)
        .depositCollateral(invId, usdc.target, ethers.parseUnits("5000", 6));
      // Dispute and resolve
    ...
    it("markOverdue on SETTLED/CANCELLED invoice reverts", async function () {
      ...
      await expect(inv.connect(creditor).markOverdue(invId)).to.be.reverted;
    });
    ...
    it("initiateDispute on SETTLED invoice reverts", async function () {
      ...
      await inv.connect(debtor).repayInvoice(invId, FACE);
      await expect(inv.connect(debtor).initiateDispute(invId, "late")).to.be
        .reverted;
    });

    it("setProtocolFee > 1000 reverts", async function () {
      const { inv, admin } = await loadFixture(deployIF);
      await expect(inv.connect(admin).setProtocolFee(1001)).to.be.reverted;
    });

    it("setSupportedToken with zero address reverts", async function () {
      const { inv, admin } = await loadFixture(deployIF);
      await expect(
        inv.connect(admin).setSupportedToken(ethers.ZeroAddress, true),
    ...
    it("setBusinessRegistry zero reverts", async function () {
      const { inv, admin } = await loadFixture(deployIF);
      await expect(
        inv.connect(admin).setBusinessRegistry(ethers.ZeroAddress),
      ).to.be.revertedWithCustomError(inv, "ZeroAddress");
    });

    it("whenNotPaused modifier prevents operations when paused", async function () {
      const { inv, usdc, admin, creditor, debtor } =
        await loadFixture(deployIF);
  ...
  describe("StreamingPayments — completed stream branches", function () {
    ...
    it("whenNotPaused prevents createStream when paused", async function () {
      const { sp, token, admin, sender, recipient } =
        await loadFixture(deploySP);
      await sp.connect(admin).pause();
      await expect(
        sp
          .connect(sender)
          .createStream(recipient.address, token.target, 10000, 3600, 0),
      ).to.be.reverted;
    });
  });
  ...
  describe("PaymentChannels — zero address branch", function () {
    it("openChannel with zero partyB reverts", async function () {
      ...
      await configureMockBusinessRegistry(pc, admin, [partyA]);
  ...
  describe("FXHedgingVault — remaining branches", function () {
    ...
    it("pair with no rate submitted (rate.updatedAt == 0) reverts on createForward", async function () {
      ...
      const newPairId = ethers.keccak256(
  ...
  describe("LiquidityPool — remaining branches", function () {
    async function deployLP() {
      const [admin, treasury, lp1, lp2, flashBorrower, other] =
        await ethers.getSigners();
      const MockERC20 = await ethers.getContractFactory("MockERC20");
    ...
    it("flash loan with token1 (via approve-in-advance pattern)", async function () {
      ...
      await pool
        .connect(lp1)
        .addLiquidity(poolId, liquidityAmt, liquidityAmt, -100, 100);
      ...
      await expect(
        pool
          ...
          .flashLoan(poolId, token1.target, loanAmt, "0x"),
      ).to.be.reverted; // covers the token1 branch in the ternary
    });

    it("updatePoolConfig rejects a flash fee above the maximum", async function () {
      ...
      await expect(pool.connect(admin).updatePoolConfig(poolId, 30, 51))
  ...
  describe("CrossChainRouter — remaining branches", function () {
    it("setTokenSupport with zero address reverts", async function () {
      ...
      await expect(
        ccr.connect(admin).setTokenSupport(ethers.ZeroAddress, true),
    ...
    it("whenNotPaused blocks initiateTransfer when paused", async function () {
      ...
      await expect(
        ccr.connect(sender).initiateTransfer(token.target, 1000, 1, rh),
      ).to.be.reverted;
    });
  });
  ...
  describe("MultiSigTreasury — remaining branches", function () {
    ...
    it("recurring payment with WEEKLY frequency", async function () {
      ...
      const tx = await createAuthorizedRecurringPayment(mst, admin, [
        ...
        "weekly",
        2,
        ethers.ZeroHash, // frequency=1 is WEEKLY
      ]);
      const r = await tx.wait();
    ...
    it("recurring payment with BIWEEKLY frequency", async function () {
      ...
      const tx = await createAuthorizedRecurringPayment(mst, admin, [
        ...
        "biweekly",
        2,
        ethers.ZeroHash, // frequency=2 is BIWEEKLY
      ]);
      const r = await tx.wait();
      const rpId = r.logs.find(
        (l) => l.fragment && l.fragment.name === "RecurringPaymentCreated",
    ...
    it("recurring payment with MONTHLY frequency", async function () {
      ...
      const tx = await createAuthorizedRecurringPayment(mst, admin, [
        ...
        "monthly",
        2,
        ethers.ZeroHash, // frequency=3 is MONTHLY
      ]);
      const r = await tx.wait();
      const rpId = r.logs.find(
        (l) => l.fragment && l.fragment.name === "RecurringPaymentCreated",
    ...
    it("updateSignerConfig with small > medium reverts", async function () {
      ...
      await expect(
        mst.connect(admin).updateSignerConfig(3, 2, 4, 4),
  ...
  // ═══════════════════════════════════════════════════════════════
  // NoblePay — monthly limit and native-asset rejection
  // ═══════════════════════════════════════════════════════════════
  describe("NoblePay — remaining branches", function () {
    async function deployNP() {
      ...
      await registry.setBusiness(sender.address, true, 2);
    ...
    it("rejects native payment with value", async function () {
    ...
    it("rejects native payment without value", async function () {
      const { np, sender, recipient } = await loadFixture(deployNP);
      const amt = 1000000n;
      await expect(
        np
    ...
    it("rejects batch with native token", async function () {
    ...
    it("whenNotPaused blocks initiatePayment", async function () {
      const { np, usdc, admin, sender, recipient } =
