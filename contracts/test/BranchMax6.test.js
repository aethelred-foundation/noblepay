...
describe("BranchMax6", function () {
  ...
  describe("PaymentChannels — channel lifecycle deep branches", function () {
    ...
    it("fundChannel on CLOSED channel reverts", async function () {
      ...
      await expect(pc.connect(partyA).fundChannel(channelId, deposit)).to.be.reverted;
    });
    ...
    it("cooperativeClose on non-ACTIVE/FUNDED channel reverts", async function () {
      ...
      await expect(pc.connect(partyA).cooperativeClose(channelId, balA, balB, 2, sigA, sigB)).to.be.reverted;
    });
    ...
    it("initiateUnilateralClose on non-ACTIVE/FUNDED channel reverts", async function () {
      ...
      await expect(pc.connect(partyA).initiateUnilateralClose(channelId, balA, balB, 2, sig2)).to.be.reverted;
    });
    ...
    it("cooperativeClose with invalid balances reverts", async function () {
      ...
      const sigA = await partyA.signMessage(ethers.getBytes(stateHash));
      const sigB = await partyB.signMessage(ethers.getBytes(stateHash));
      await expect(pc.connect(partyA).cooperativeClose(channelId, balA, balB, nonce, sigA, sigB)).to.be.reverted;
    });
    ...
    it("counterDispute on non-CLOSING channel reverts", async function () {
      ...
      await expect(pc.connect(partyB).counterDispute(channelId, 0, 0, 1, sig)).to.be.reverted;
    });
    ...
    it("HTLC claim with wrong preimage reverts", async function () {
      ...
      await expect(pc.connect(partyB).claimHTLC(htlcId, wrongPreimage)).to.be.reverted;
    });
    ...
    it("createHTLC with timelock too short reverts", async function () {
      ...
      const now = await time.latest();
      await expect(pc.connect(partyA).createHTLC(channelId, 1000, hashLock, now + 100)).to.be.reverted;
    });
    ...
    it("createHTLC with timelock too long reverts", async function () {
      ...
      const now = await time.latest();
      await expect(pc.connect(partyA).createHTLC(channelId, 1000, hashLock, now + 31 * 86400)).to.be.reverted;
    });
    ...
    it("createHTLC with insufficient balance reverts", async function () {
      ...
      await expect(pc.connect(partyA).createHTLC(channelId, tooMuch, hashLock, now + 7200)).to.be.reverted;
    });
    ...
    it("watchtower double registration reverts", async function () {
      ...
      await pc.connect(watchtower1).registerWatchtower(100, { value: ethers.parseEther("1") });
      await expect(pc.connect(watchtower1).registerWatchtower(100, { value: ethers.parseEther("1") })).to.be.reverted;
    });

    // 27. Watchtower insufficient stake
    it("watchtower insufficient stake reverts", async function () {
      const { pc, watchtower1 } = await loadFixture(deployPC);
      await expect(pc.connect(watchtower1).registerWatchtower(100, { value: ethers.parseEther("0.5") })).to.be.reverted;
    });
    ...
    it("deregister non-existent watchtower reverts", async function () {
      ...
      await expect(pc.connect(other).deregisterWatchtower()).to.be.reverted;
    });

    // 29. Watchtower bounty too high
    it("watchtower bounty too high reverts", async function () {
      const { pc, watchtower1 } = await loadFixture(deployPC);
      await expect(pc.connect(watchtower1).registerWatchtower(600, { value: ethers.parseEther("1") })).to.be.reverted;
    });
    ...
    it("registerRoutingPath too many hops reverts", async function () {
      ...
      await expect(pc.connect(router).registerRoutingPath(fakeChannels, fakeIntermediaries, 100)).to.be.reverted;
    });

    // 32. Routing path empty
    it("registerRoutingPath empty reverts", async function () {
      const { pc, router } = await loadFixture(deployPC);
      await expect(pc.connect(router).registerRoutingPath([], [], 100)).to.be.reverted;
    });
    ...
    it("completeRoutingPath already completed reverts", async function () {
      ...
      const pathId = r.logs.find(l => l.fragment && l.fragment.name === "RoutingPathCreated").args[0];
      await pc.connect(router).completeRoutingPath(pathId);
      await expect(pc.connect(router).completeRoutingPath(pathId)).to.be.reverted;
    });
    ...
    it("batchOpenChannels too many reverts", async function () {
      ...
      await expect(pc.connect(partyA).batchOpenChannels(parties, usdc.target, deposits, 3600, 100)).to.be.reverted;
    });
    ...
    it("batchSetKYCStatus with zero address reverts", async function () {
      ...
      await expect(pc.connect(admin).batchSetKYCStatus([ethers.ZeroAddress], [true])).to.be.reverted;
    });
    ...
    it("openChannel with self as partyB reverts", async function () {
      ...
      await expect(pc.connect(partyA).openChannel(partyA.address, usdc.target, 1000, 3600, 100)).to.be.reverted;
    });
    ...
    it("openChannel with zero partyB reverts", async function () {
      ...
      await expect(pc.connect(partyA).openChannel(ethers.ZeroAddress, usdc.target, 1000, 3600, 100)).to.be.reverted;
    });
    ...
    it("openChannel with unsupported token reverts", async function () {
      ...
      await expect(pc.connect(partyA).openChannel(partyB.address, fakeToken, 1000, 3600, 100)).to.be.reverted;
    });

    // 42. openChannel with invalid challenge period
    it("openChannel with challenge period too short reverts", async function () {
      const { pc, usdc, partyA, partyB } = await loadFixture(deployPC);
      await expect(pc.connect(partyA).openChannel(partyB.address, usdc.target, 1000, 60, 100)).to.be.reverted;
    });

    // 43. openChannel with challenge period too long
    it("openChannel with challenge period too long reverts", async function () {
      const { pc, usdc, partyA, partyB } = await loadFixture(deployPC);
      await expect(pc.connect(partyA).openChannel(partyB.address, usdc.target, 1000, 8 * 86400, 100)).to.be.reverted;
    });

    // 44. openChannel with routing fee too high
    it("openChannel with routing fee too high reverts", async function () {
      const { pc, usdc, partyA, partyB } = await loadFixture(deployPC);
      await expect(pc.connect(partyA).openChannel(partyB.address, usdc.target, 1000, 3600, 600)).to.be.reverted;
    });
    ...
    it("fundChannel by non-party reverts", async function () {
      const { pc, usdc, partyA, partyB, other } = await loadFixture(deployPC);
      const channelId = await openAndActivateChannel(pc, usdc, partyA, partyB);
      await expect(pc.connect(other).fundChannel(channelId, 1000)).to.be.reverted;
    });
    ...
    it("initiateUnilateralClose with invalid balances reverts", async function () {
      ...
      const sigB = await partyB.signMessage(ethers.getBytes(stateHash));
      await expect(pc.connect(partyA).initiateUnilateralClose(channelId, 0, 0, 1, sigB)).to.be.reverted;
    });
    ...
    it("counterDispute with low nonce reverts", async function () {
      ...
      const sigA = await partyA.signMessage(ethers.getBytes(stateHash));
      await expect(pc.connect(partyB).counterDispute(channelId, deposit, deposit, 1, sigA)).to.be.reverted;
    });
    ...
    it("counterDispute after challenge period expired reverts", async function () {
      ...
      await expect(pc.connect(partyB).counterDispute(channelId, deposit, deposit, 2, sigA)).to.be.reverted;
    });
    ...
    it("paused contract blocks openChannel", async function () {
      ...
      await expect(pc.connect(partyA).openChannel(partyB.address, usdc.target, 1000, 3600, 100)).to.be.reverted;
    ...
    it("setNoblePayContract zero address reverts", async function () {
      const { pc, admin } = await loadFixture(deployPC);
      await expect(pc.connect(admin).setNoblePayContract(ethers.ZeroAddress)).to.be.reverted;
    });
    ...
    it("constructor with zero admin reverts", async function () {
      ...
      await expect(PC.deploy(ethers.ZeroAddress, treasury.address, 50)).to.be.reverted;
    });
    ...
    it("constructor with zero treasury reverts", async function () {
      ...
      await expect(PC.deploy(admin.address, ethers.ZeroAddress, 50)).to.be.reverted;
    });
    ...
    it("constructor with fee too high reverts", async function () {
      ...
      await expect(PC.deploy(admin.address, treasury.address, 600)).to.be.reverted;
    });
    ...
    it("openChannel with non-KYC partyB reverts", async function () {
      ...
      await expect(pc.connect(partyA).openChannel(other.address, usdc.target, 1000, 3600, 100)).to.be.reverted;
    });
    ...
    it("counterDispute with invalid balances reverts", async function () {
      ...
      await expect(pc.connect(partyB).counterDispute(channelId, 1, 1, 2, sigA)).to.be.reverted;
    });
  });
  ...
  describe("ComplianceOracle — additional branches", function () {
    ...
    it("registerTEENode with insufficient stake reverts", async function () {
      ...
      await expect(co.connect(tee1).registerTEENode(key, platformId, { value: ethers.parseEther("5") })).to.be.reverted;
    });
    ...
    it("deregisterTEENode of non-existent node reverts", async function () {
      ...
      await expect(co.connect(admin).deregisterTEENode(other.address)).to.be.reverted;
    });
  ...
  describe("TravelRule — additional branches", function () {
    ...
    it("registerVASP with empty encryption key reverts", async function () {
      ...
      await expect(tr.connect(vasp1).registerVASP(ethers.ZeroHash, "0x")).to.be.reverted;
    });
    ...
    it("double registration reverts", async function () {
      ...
      await expect(tr.connect(vasp1).registerVASP(ethers.keccak256(ethers.toUtf8Bytes("inst2")), key)).to.be.reverted;
    });
  ...
  describe("FXHedgingVault — additional branches", function () {
    ...
    it("createForward with zero notional reverts", async function () {
      ...
      const now = await time.latest();
      await expect(fx.connect(hedger).createForward(pairId, 0, now + 30 * 86400, usdc.target, ethers.parseUnits("1000", 6))).to.be.reverted;
    });

    it("createForward with maturity in past reverts", async function () {
      const { fx, usdc, hedger, pairId } = await loadFixture(deployFX);
      const now = await time.latest();
      await expect(fx.connect(hedger).createForward(pairId, ethers.parseUnits("1000", 6), now - 100, usdc.target, ethers.parseUnits("1000", 6))).to.be.reverted;
    });
    ...
    it("createForward with unsupported collateral reverts", async function () {
      ...
      const now = await time.latest();
      const fakeToken = ethers.Wallet.createRandom().address;
      await expect(fx.connect(hedger).createForward(pairId, ethers.parseUnits("1000", 6), now + 30 * 86400, fakeToken, ethers.parseUnits("1000", 6))).to.be.reverted;
    });
    ...
    it("createForward on non-existent pair reverts", async function () {
      ...
      await expect(fx.connect(hedger).createForward(fakePair, ethers.parseUnits("1000", 6), now + 30 * 86400, usdc.target, ethers.parseUnits("1000", 6))).to.be.reverted;
    });
  ...
  describe("LiquidityPool — pool health branches", function () {
    ...
    it("createPool with token0 > token1 (wrong order) reverts", async function () {
      ...
      await expect(pool.connect(admin).createPool(token1.target, token0.target, 30, 10, 500)).to.be.reverted;
    });

    it("createPool with same token reverts", async function () {
      const { pool, token0, admin } = await loadFixture(deployLP);
      await expect(pool.connect(admin).createPool(token0.target, token0.target, 30, 10, 500)).to.be.reverted;
    });
    ...
    it("addLiquidity with zero amounts reverts", async function () {
      ...
      const r = await tx.wait();
      const poolId = r.logs.find(l => l.fragment && l.fragment.name === "PoolCreated").args[0];
      await expect(pool.connect(lp1).addLiquidity(poolId, 0, 0, -1000, 1000)).to.be.reverted;
    });
  });
  ...
  describe("StreamingPayments — additional branches", function () {
    ...
    it("createStream with zero amount reverts", async function () {
      const { sp, usdc, sender, recipient } = await loadFixture(deploySP);
      await expect(sp.connect(sender).createStream(recipient.address, usdc.target, 0, 3600, 0)).to.be.reverted;
    });

    it("createStream to zero address reverts", async function () {
      const { sp, usdc, sender } = await loadFixture(deploySP);
      await expect(sp.connect(sender).createStream(ethers.ZeroAddress, usdc.target, 1000, 3600, 0)).to.be.reverted;
    });

    it("createStream to self reverts", async function () {
      const { sp, usdc, sender } = await loadFixture(deploySP);
      await expect(sp.connect(sender).createStream(sender.address, usdc.target, 1000, 3600, 0)).to.be.reverted;
    });
  ...
  describe("CrossChainRouter — reputation zero auto-deactivation", function () {
    ...
    it("addChain duplicate reverts", async function () {
      const { ccr, admin } = await loadFixture(deployCCR);
      await expect(ccr.connect(admin).addChain(137, "Polygon2", ethers.parseUnits("1", 6), 10, 256, 86400, ethers.parseUnits("10", 6), ethers.parseUnits("1000000", 6))).to.be.reverted;
    });
    ...
    it("initiateTransfer with unsupported token reverts", async function () {
      ...
      await expect(ccr.connect(other).initiateTransfer(fakeToken, 1000, 137, recipientHash)).to.be.reverted;
    });

    it("initiateTransfer below minimum reverts", async function () {
      const { ccr, usdc, other } = await loadFixture(deployCCR);
      const recipientHash = ethers.keccak256(ethers.toUtf8Bytes("recipient"));
      await expect(ccr.connect(other).initiateTransfer(usdc.target, 1, 137, recipientHash)).to.be.reverted;
    });
    ...
    it("initiateTransfer above maximum reverts", async function () {
      ...
      await usdc.connect(other).approve(ccr.target, ethers.MaxUint256);
      await expect(ccr.connect(other).initiateTransfer(usdc.target, tooMuch, 137, recipientHash)).to.be.reverted;
    });
  });
  ...
  describe("NoblePay — deeper branches", function () {
    ...
    it("initiatePayment with zero amount reverts", async function () {
      ...
      await expect(np.connect(sender).initiatePayment(recipient.address, 0, usdc.target, PURPOSE, "0x414544")).to.be.reverted;
    });

    it("initiatePayment with unsupported token reverts", async function () {
      const { np, sender, recipient } = await loadFixture(deployNP);
      const fakeToken = ethers.Wallet.createRandom().address;
      await expect(np.connect(sender).initiatePayment(recipient.address, 1000, fakeToken, PURPOSE, "0x414544")).to.be.reverted;
    });
    ...
    it("initiatePayment by unregistered business reverts", async function () {
      ...
      await expect(np.connect(other).initiatePayment(recipient.address, 1000, usdc.target, PURPOSE, "0x414544")).to.be.reverted;
    });

    it("initiatePayment to zero address reverts", async function () {
      const { np, usdc, sender } = await loadFixture(deployNP);
      await expect(np.connect(sender).initiatePayment(ethers.ZeroAddress, 1000, usdc.target, PURPOSE, "0x414544")).to.be.reverted;
    });
  ...
  describe("MultiSigTreasury — deeper branches", function () {
    ...
    it("executeProposal that hasn't reached threshold reverts", async function () {
      ...
      await expect(mst.connect(signer1).executeProposal(proposalId)).to.be.reverted;
    });
    ...
    it("addSigner that is already a signer reverts", async function () {
      ...
      await expect(mst.connect(admin).addSigner(signer1.address)).to.be.reverted;
    });
  });
  ...
  describe("BusinessRegistry — additional branches", function () {
    ...
    it("registerBusiness with zero compliance officer reverts", async function () {
      ...
      await expect(br.connect(business1).registerBusiness("LIC001", "TestBiz", 0, ethers.ZeroAddress)).to.be.reverted;
    });

    it("verifyBusiness for non-existent business reverts", async function () {
      const { br, verifier, other } = await loadFixture(deployBR);
      await expect(br.connect(verifier).verifyBusiness(other.address)).to.be.reverted;
    });

    it("suspendBusiness for non-existent business reverts", async function () {
      const { br, verifier, other } = await loadFixture(deployBR);
      // suspendBusiness(address, reason)
      await expect(br.connect(verifier).suspendBusiness(other.address, "test reason")).to.be.reverted;
    });
  ...
  describe("InvoiceFinancing — credit score and penalty branches", function () {
    ...
    it("createInvoice with zero face value reverts", async function () {
      ...
      const now = await time.latest();
      await expect(inv.connect(creditor).createInvoice(debtor.address, 0, usdc.target, now + 86400, ethers.ZeroHash, 0, 0)).to.be.reverted;
    });

    it("createInvoice with zero debtor reverts", async function () {
      const { inv, usdc, creditor } = await loadFixture(deployIF);
      const now = await time.latest();
      await expect(inv.connect(creditor).createInvoice(ethers.ZeroAddress, 1000, usdc.target, now + 86400, ethers.ZeroHash, 0, 0)).to.be.reverted;
    });
    ...
    it("financeInvoice with zero amount reverts", async function () {
      ...
      await expect(inv.connect(factor).financeInvoice(invoiceId, 0, 500)).to.be.reverted;
    });
