...
describe("MultiSigTreasury - Coverage", function () {
  async function deployFixture() {
    ...
    await treasury
      .connect(admin)
      .grantRole(await treasury.BUDGET_MANAGER_ROLE(), budgetMgr.address);
    await treasury
      .connect(admin)
      .grantRole(await treasury.YIELD_MANAGER_ROLE(), yieldMgr.address);
  ...
  describe("Constructor Edge Cases", function () {
    ...
    it("should revert with small threshold = 0", async function () {
      ...
      await expect(
        Treasury.deploy(admin.address, [s1.address, s2.address], 0, 1, 2, 2),
    ...
    it("should revert with small > medium threshold", async function () {
      ...
      const [admin, s1, s2] = await ethers.getSigners();
      await expect(
        Treasury.deploy(admin.address, [s1.address, s2.address], 2, 1, 2, 2),
      ).to.be.revertedWithCustomError(Treasury, "InvalidSignerConfig");
    });
  ...
  describe("Medium Tier Proposals", function () {
    it("should classify medium tier ($10K-$100K)", async function () {
      const { treasury, usdc, admin, signer1, recipient } =
  ...
  describe("Emergency Proposals", function () {
    it("should create emergency proposal with fast-track timelock", async function () {
      const { treasury, usdc, signer1, signer2, signer3, signer4, recipient } =
        await loadFixture(deployFixture);
  ...
  describe("Proposal Execution - Native Tokens", function () {
    it("should execute proposal with native tokens", async function () {
      const { treasury, signer1, signer2, recipient, admin } =
        await loadFixture(deployFixture);
    ...
    it("should revert native execution with insufficient balance", async function () {
      const { treasury, signer1, signer2, recipient } =
        await loadFixture(deployFixture);
  ...
  describe("Proposal Execution with Budget", function () {
    it("should execute proposal linked to a budget", async function () {
      const { treasury, usdc, signer1, signer2, budgetMgr, recipient } =
        await loadFixture(deployFixture);
      const periodEnd = BigInt(await time.latest()) + 86400n * 365n;
      const budgetTx = await treasury
        .connect(budgetMgr)
        .createBudget(
          "Engineering",
          5,
      ...
      const budgetEvent = budgetReceipt.logs.find(
  ...
  describe("Cancel Proposal", function () {
    it("should allow admin to cancel a proposal", async function () {
      const { treasury, usdc, signer1, admin, recipient } =
        await loadFixture(deployFixture);
      const tx = await treasury
        .connect(signer1)
        .createProposal(
      ...
      await expect(treasury.connect(admin).cancelProposal(proposalId)).to.emit(
        treasury,
        "ProposalCancelled",
      );
    });
    ...
    it("should allow cancel of approved proposal", async function () {
      const { treasury, usdc, signer1, signer2, admin, recipient } =
        await loadFixture(deployFixture);
      const tx = await treasury
        .connect(signer1)
        .createProposal(
    ...
    it("should revert cancel by unauthorized", async function () {
      const { treasury, usdc, signer1, other, recipient } =
        await loadFixture(deployFixture);
      const tx = await treasury
        .connect(signer1)
        .createProposal(
      ...
      await expect(
        treasury.connect(other).cancelProposal(proposalId),
    ...
    it("should revert cancel of executed proposal", async function () {
      ...
      await expect(
        treasury.connect(admin).cancelProposal(proposalId),
  ...
  describe("Recurring Payments - Native & Budget", function () {
    it("should execute recurring payment with native tokens", async function () {
      const { treasury, signer1, recipient, admin } =
        await loadFixture(deployFixture);
      await admin.sendTransaction({
        to: treasury.target,
        value: ethers.parseEther("10"),
      ...
      const tx = await createAuthorizedRecurringPayment(treasury, admin, [
        recipient.address,
        ethers.ZeroAddress,
        amount,
        0,
    ...
    it("should execute recurring payment linked to budget", async function () {
      const { treasury, usdc, admin, signer1, budgetMgr, recipient } =
        await loadFixture(deployFixture);
      const periodEnd = BigInt(await time.latest()) + 86400n * 365n;
      const budgetTx = await treasury
        .connect(budgetMgr)
    ...
    it("should revert when max executions reached", async function () {
      ...
      await time.increase(24 * 3600 + 1);
      await expect(
        treasury.connect(signer1).executeRecurringPayment(paymentId),
  ...
  describe("Yield Management", function () {
    it("should allocate funds to approved yield protocol", async function () {
      const { treasury, usdc, admin, yieldMgr, other } =
        await loadFixture(deployFixture);
      await treasury
        .connect(admin)
      ...
      await expect(
        treasury
          .connect(yieldMgr)
          .allocateToYield(other.address, usdc.target, amount),
      ).to.emit(treasury, "YieldAllocated");
    });

    it("should revert allocation to unapproved protocol", async function () {
      const { treasury, usdc, yieldMgr, other } =
        await loadFixture(deployFixture);
      await expect(
        treasury
          .connect(yieldMgr)
          .allocateToYield(other.address, usdc.target, 1000),
    ...
    it("should revert zero amount allocation", async function () {
      ...
      await expect(
        treasury
          .connect(yieldMgr)
          .allocateToYield(other.address, usdc.target, 0),
  ...
  describe("Signer Management", function () {
    it("should add and remove a signer", async function () {
      ...
      await expect(treasury.connect(admin).addSigner(other.address)).to.emit(
        treasury,
        "SignerAdded",
      );
      ...
      await expect(treasury.connect(admin).removeSigner(other.address)).to.emit(
        treasury,
        "SignerRemoved",
      );
    ...
    it("should revert adding existing signer", async function () {
      ...
      await expect(
        treasury.connect(admin).addSigner(signer1.address),
    ...
    it("should revert adding zero address signer", async function () {
      ...
      await expect(
        treasury.connect(admin).addSigner(ethers.ZeroAddress),
      ).to.be.revertedWithCustomError(treasury, "ZeroAddress");
    });
    ...
    it("should remove a signer that is not the last in array (swap-and-pop)", async function () {
      const { treasury, admin, signer1, signer2, other } =
        await loadFixture(deployFixture);
  ...
  describe("Update Signer Config", function () {
    it("should update signer config", async function () {
      const { treasury, admin } = await loadFixture(deployFixture);
      await expect(
        treasury.connect(admin).updateSignerConfig(1, 2, 3, 3),
    ...
    it("should revert invalid signer config update", async function () {
      const { treasury, admin } = await loadFixture(deployFixture);
      await expect(
        treasury.connect(admin).updateSignerConfig(0, 2, 3, 3),
      ).to.be.revertedWithCustomError(treasury, "InvalidSignerConfig");
    });
    ...
    it("should revert when large > totalSigners", async function () {
      const { treasury, admin } = await loadFixture(deployFixture);
      await expect(
        treasury.connect(admin).updateSignerConfig(1, 2, 6, 6),
      ).to.be.revertedWithCustomError(treasury, "InvalidSignerConfig");
    });
  });
  ...
  describe("View Functions", function () {
    ...
    it("should check hasApproved", async function () {
      ...
      expect(await treasury.hasApproved(proposalId, signer1.address)).to.be
        .true;
    });
  ...
  describe("Admin - setNoblePayContract", function () {
    it("should set NoblePay contract", async function () {
      const { treasury, admin, other } = await loadFixture(deployFixture);
      await expect(
        treasury.connect(admin).setNoblePayContract(other.address),
    ...
    it("should revert for zero address", async function () {
      const { treasury, admin } = await loadFixture(deployFixture);
      await expect(
        treasury.connect(admin).setNoblePayContract(ethers.ZeroAddress),
      ).to.be.revertedWithCustomError(treasury, "ZeroAddress");
    });
  });
  ...
  describe("Proposal Creation Edge Cases", function () {
    ...
    it("should revert with unsupported token", async function () {
      const { treasury, signer1, recipient, other } =
        await loadFixture(deployFixture);
      await expect(
        treasury
          .connect(signer1)
  ...
  describe("Recurring Payment Edge Cases", function () {
    it("should revert with zero recipient", async function () {
      const { treasury, usdc, admin, signer1 } =
        await loadFixture(deployFixture);
      await expect(
        treasury
          .connect(admin)
  ...
  describe("Delegation Edge Cases", function () {
    it("should revert delegation with zero address", async function () {
      ...
      await expect(
        treasury
          .connect(signer1)
          .delegateSigningAuthority(ethers.ZeroAddress, 86400),
      ).to.be.revertedWithCustomError(treasury, "ZeroAddress");
    });
  });
