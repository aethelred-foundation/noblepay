...
describe("StreamingPayments - Branch Coverage", function () {
  ...
  it("should revert batch streams with empty array", async function () {
    ...
    await expect(
      sp
        .connect(sender)
        .createBatchStreams([], token.target, [], 86400 * 30, 0),
  ...
  it("should pause stream by sender", async function () {
    ...
    await expect(sp.connect(sender).pauseStream(streamId)).to.emit(
      sp,
      "StreamPaused",
    );
  });
  ...
  it("should pause stream by admin", async function () {
    ...
    await expect(sp.connect(admin).pauseStream(streamId)).to.emit(
      sp,
      "StreamPaused",
    );
  });
  ...
  it("should resume a paused stream", async function () {
    ...
    await expect(sp.connect(sender).resumeStream(streamId)).to.emit(
      sp,
      "StreamResumed",
    );
  });
  ...
  it("should cancel a stream", async function () {
    ...
    await expect(sp.connect(sender).cancelStream(streamId)).to.emit(
      sp,
      "StreamCancelled",
    );
  });
  ...
  it("should revert withdraw before cliff", async function () {
    ...
    await expect(
      sp.connect(recipient).withdraw(streamId),
  ...
  it("should withdraw after cliff", async function () {
    ...
    await expect(sp.connect(recipient).withdraw(streamId)).to.emit(
      sp,
      "Withdrawal",
    );
  });
...
describe("ComplianceOracle - Branch Coverage", function () {
  async function deployFixture() {
    const [admin, teeManager, operator, operator2, other] =
  ...
  it("should deregister TEE node by self", async function () {
    ...
    await expect(
      co.connect(operator).deregisterTEENode(operator.address),
  ...
  it("should deregister TEE node by manager", async function () {
    ...
    await expect(
      co.connect(teeManager).deregisterTEENode(operator.address),
    ).to.emit(co, "TEENodeDeregistered");
  });
  ...
  it("should revert deregister by unauthorized", async function () {
    ...
    await expect(
      co.connect(other).deregisterTEENode(operator.address),
  ...
  it("should slash an offline node", async function () {
    ...
    await expect(
      co.connect(teeManager).slashOfflineNode(operator.address),
  ...
  it("should verify attestation", async function () {
    ...
    await co
      .connect(teeManager)
      .verifyAttestation(operator.address, attestData, expectedHash);
  });
});
...
describe("CrossChainRouter - Branch Coverage", function () {
  ...
  it("should revert transfer below minimum amount", async function () {
    ...
    await expect(
      router
        .connect(sender)
        .initiateTransfer(usdc.target, 1, CHAIN_ID, ethers.keccak256("0x01")),
  ...
  it("should recover a timed-out transfer by sender", async function () {
    ...
    await expect(router.connect(sender).recoverTransfer(transferId)).to.emit(
      router,
      "TransferRecovered",
    );
  });
  ...
  it("should recover a timed-out transfer by admin", async function () {
    ...
    await time.increase(RECOVERY_TIMEOUT + 1);
    await expect(router.connect(admin).recoverTransfer(transferId)).to.emit(
      router,
      "TransferRecovered",
    );
  });
  ...
  it("should revert recovery by unauthorized", async function () {
    ...
    await expect(
      router.connect(other).recoverTransfer(transferId),
  ...
  it("should mark transfer as failed", async function () {
    const { router, admin, transferId } = await transferFixture();
    await expect(
      router.connect(admin).markTransferFailed(transferId, "network error"),
...
describe("TravelRule - Branch Coverage", function () {
  ...
  it("should deactivate a VASP", async function () {
    ...
    await expect(tr.connect(admin).deactivateVASP(vasp1.address)).to.emit(
      tr,
      "VASPDeactivated",
    );
  });
  ...
  it("should verify travel rule compliance", async function () {
    ...
    await expect(
      tr.connect(teeNode).verifyTravelRuleCompliance(travelRuleId),
  ...
  it("should reject travel rule data", async function () {
    const { tr, teeNode, travelRuleId } = await travelRuleSubmittedFixture();
    await expect(
      tr.connect(teeNode).rejectTravelRuleData(travelRuleId, "Invalid data"),
    ).to.emit(tr, "TravelRuleRejected");
  });

  it("should share with receiving institution", async function () {
    const { tr, teeNode, vasp1, vasp2, travelRuleId } =
      await travelRuleSubmittedFixture();
  ...
  it("should acknowledge travel rule data", async function () {
    ...
    const shareEvent = shareReceipt.logs.find(
      (l) => l.fragment && l.fragment.name === "TravelRuleShared",
    );
    const sharingId = shareEvent.args[1];

    await expect(
      tr.connect(vasp2).acknowledgeTravelRuleData(sharingId),
  ...
  it("should check requiresFullTravelRuleData", async function () {
    ...
    const needsFull = await tr.requiresFullTravelRuleData(
...
describe("AIComplianceModule - Branch Coverage", function () {
  ...
  async function deployFixture() {
    ...
    await aic
      .connect(admin)
      .grantRole(await aic.AI_OPERATOR_ROLE(), aiOp.address);
    await aic
      .connect(admin)
      .grantRole(await aic.COMPLIANCE_OFFICER_ROLE(), officer.address);

    const modelHash = ethers.keccak256(ethers.toUtf8Bytes("model_v1"));
    const tx = await aic
      .connect(aiOp)
      .registerModel("AML-Detector", "1.0", modelHash);
    const receipt = await tx.wait();
    const event = receipt.logs.find(
  ...
  async function decisionFixture() {
    ...
    const tx = await aic
      .connect(aiOp)
      .recordDecision(subjectHash, modelId, 1, 85, evidenceHash, reasonHash);
    const receipt = await tx.wait();
    const event = receipt.logs.find(
  ...
  it("should file an appeal", async function () {
    ...
    await expect(
      aic
        ...
        .fileAppeal(
          decisionId,
          ethers.keccak256(ethers.toUtf8Bytes("grounds")),
        ),
  ...
  it("should start appeal review", async function () {
    ...
    await expect(aic.connect(officer).startAppealReview(appealId)).to.emit(
      aic,
      "AppealReviewStarted",
    );
  });
  ...
  it("should resolve appeal with UPHELD", async function () {
    ...
    await expect(
      aic
        .connect(officer)
        .resolveAppeal(appealId, 3, 1, ethers.keccak256("0x03")),
  ...
  it("should resolve appeal with OVERTURNED", async function () {
    ...
    await expect(
      aic
        .connect(officer)
        .resolveAppeal(appealId, 4, 0, ethers.keccak256("0x03")),
    ).to.emit(aic, "AppealResolved");
  });
  ...
  it("should update model status", async function () {
    const { aic, aiOp, modelId } = await loadFixture(deployFixture);
    await expect(aic.connect(aiOp).updateModelStatus(modelId, 1)).to.emit(
      aic,
      "ModelStatusUpdated",
    );
  });
...
describe("BusinessRegistry - Branch Coverage", function () {
  ...
  it("should suspend a verified business", async function () {
    ...
    await expect(
      br.connect(verifier).suspendBusiness(biz1.address, "AML violation"),
  ...
  it("should revert suspending non-verified business", async function () {
    ...
    await expect(
      br.connect(verifier).suspendBusiness(biz1.address, "test"),
  ...
  it("should reinstate a suspended business", async function () {
    ...
    await expect(br.connect(verifier).reinstateBusiness(biz1.address)).to.emit(
      br,
      "BusinessReinstated",
    );
  });
  ...
  it("should revert reinstate of non-suspended business", async function () {
    const { br, verifier, biz1 } = await verifiedFixture();
    await expect(
      br.connect(verifier).reinstateBusiness(biz1.address),
    ).to.be.revertedWithCustomError(br, "InvalidKYCStatus");
  });
  ...
  it("should revoke a business", async function () {
    ...
    await expect(
      br.connect(admin).revokeBusiness(biz1.address, "fraud"),
    ).to.emit(br, "BusinessRevoked");
  });

  it("should upgrade business tier", async function () {
    const { br, admin, biz1 } = await verifiedFixture();
    await expect(br.connect(admin).upgradeTier(biz1.address, 1)).to.emit(
      br,
      "TierUpgraded",
    );
  });
  ...
  it("should revert tier downgrade", async function () {
    ...
    await expect(
      br.connect(admin).upgradeTier(biz1.address, 0),
  ...
  it("should update compliance officer", async function () {
    ...
    await expect(
      br.connect(biz1).updateComplianceOfficer(officer2.address),
  ...
  it("should revert duplicate license registration", async function () {
    ...
    await expect(
      br
        .connect(biz2)
        .registerBusiness("ABC123", "Other Corp", 1, officer1.address),
...
describe("PaymentChannels - Branch Coverage", function () {
  async function deployFixture() {
    const [admin, partyA, partyB, router, treasury, other] =
      await ethers.getSigners();
    const MockERC20 = await ethers.getContractFactory("MockERC20");
    const usdc = await MockERC20.deploy("USDC", "USDC", 6);
  ...
  it("should initiate unilateral close", async function () {
    ...
    await expect(
      pc
        .connect(partyA)
        .initiateUnilateralClose(channelId, balA, balB, nonce, sig),
...
describe("InvoiceFinancing - Branch Coverage", function () {
  async function deployFixture() {
    ...
    await invoicing
      .connect(admin)
      .grantRole(CREDIT_ANALYST_ROLE, analyst.address);
  ...
  it("should create a chained invoice", async function () {
    const { invoicing, token, creditor, debtor, other, invoiceId } =
      await invoiceFixture();
    const maturity = BigInt(await time.latest()) + 86400n * 90n;
  ...
  it("should revert invoice with zero face value", async function () {
    const { invoicing, token, creditor, debtor } =
      await loadFixture(deployFixture);
    const maturity = BigInt(await time.latest()) + 86400n * 90n;
    await expect(
      invoicing
  ...
  it("should initiate dispute by debtor", async function () {
    ...
    await expect(
      invoicing.connect(debtor).initiateDispute(invoiceId, "disputed"),
  ...
  it("should batch create invoices", async function () {
    const { invoicing, token, creditor, debtor, other } =
      await loadFixture(deployFixture);
    const maturity = BigInt(await time.latest()) + 86400n * 90n;
  ...
  it("should revert batch with empty array", async function () {
    ...
    await expect(
      invoicing
        .connect(creditor)
        .batchCreateInvoices([], [], token.target, [], [], 7, 200),
    ).to.be.revertedWithCustomError(invoicing, "ZeroAmount");
  });
});
