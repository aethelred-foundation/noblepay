...
describe("BranchMax8 — Final coverage push", function () {
  ...
  describe("ComplianceOracle — remaining branches", function () {
    async function deployCO() {
      const [admin, tee1, tee2, thresholdMgr, other] =
      ...
      await co
        .connect(admin)
        .grantRole(await co.THRESHOLD_MANAGER_ROLE(), thresholdMgr.address);
    ...
    it("deregisterTEENode from non-admin reverts", async function () {
      ...
      await expect(co.connect(other).deregisterTEENode(tee1.address)).to.be
        .reverted;
    });
    ...
    it("threshold manager proposes threshold change", async function () {
      ...
      await expect(
        co.connect(thresholdMgr).proposeThresholdUpdate(30, 70),
      ).to.emit(co, "ThresholdChangeProposed");
    });
  });
  ...
  describe("TravelRule — deeper branches", function () {
    ...
    it("duplicate submission reverts", async function () {
      ...
      await tr
        ...
        .submitTravelRuleData(
          paymentId,
          ethers.keccak256(ethers.toUtf8Bytes("on")),
          vasp1.address,
          ethers.keccak256(ethers.toUtf8Bytes("oi")),
          ethers.keccak256(ethers.toUtf8Bytes("bn")),
          vasp2.address,
          ethers.keccak256(ethers.toUtf8Bytes("bi")),
          1000,
          "0x555344",
    ...
    it("verifyTravelRuleCompliance from non-TEE reverts", async function () {
      ...
      await expect(tr.connect(other).verifyTravelRuleCompliance(fakeId)).to.be
        .reverted;
    });
    ...
    it("verifyTravelRuleCompliance succeeds", async function () {
      ...
      const ev = r.logs.find(
        (l) => l.fragment && l.fragment.name === "TravelRuleSubmitted",
      );
      if (ev) {
    ...
    it("rejectTravelRuleData from non-TEE reverts", async function () {
      ...
      await expect(tr.connect(other).rejectTravelRuleData(fakeId, "reason")).to
        .be.reverted;
    });
    ...
    it("acknowledgeTravelRuleData from non-VASP reverts", async function () {
      ...
      await expect(tr.connect(other).acknowledgeTravelRuleData(fakeId)).to.be
        .reverted;
    });
  ...
  describe("StreamingPayments — remaining conditional branches", function () {
    ...
    it("createStream with duration below minimum reverts", async function () {
      ...
      await expect(
        sp
          .connect(sender)
          .createStream(recipient.address, usdc.target, 1000, 100, 0),
      ).to.be.reverted;
    });
  ...
  describe("FXHedgingVault — deeper branches", function () {
    async function deployFX() {
      const [admin, treasury, oracle, hedger, hedger2, other] =
        await ethers.getSigners();
      const MockERC20 = await ethers.getContractFactory("MockERC20");
      const usdc = await MockERC20.deploy("USDC", "USDC", 6);
    ...
    it("addCurrencyPair from non-admin reverts", async function () {
      ...
      await expect(
        fx
          .connect(other)
          .addCurrencyPair("0x474250", "0x555344", 500, 300, 200),
      ).to.be.reverted;
    });
    ...
    it("setSupportedCollateral from non-admin reverts", async function () {
      const { fx, other } = await loadFixture(deployFX);
      await expect(
        fx.connect(other).setSupportedCollateral(ethers.ZeroAddress, true),
      ).to.be.reverted;
    });

    it("createForward from non-owner settles to different account", async function () {
      const { fx, usdc, hedger, hedger2, pairId, oracle } =
      ...
      const r = await tx.wait();
      const posEv = r.logs.find(
        (l) =>
          l.fragment &&
          (l.fragment.name === "ForwardCreated" ||
            l.fragment.name === "PositionOpened"),
      );
      if (posEv) {
        ...
        await fx
          .connect(oracle)
          .submitFXRate(pairId, ethers.parseUnits("1.2", 8));
  ...
  describe("LiquidityPool — remaining branches", function () {
    async function deployLP() {
      ...
      await pool
        .connect(admin)
        .grantRole(await pool.LIQUIDITY_PROVIDER_ROLE(), lp2.address);
    ...
    it("addLiquidity below minimum reverts", async function () {
      ...
      await expect(pool.connect(lp1).addLiquidity(poolId, 1, 1, -100, 100)).to
        .be.reverted;
    });
    ...
    it("harvestFees on position with no fees", async function () {
      ...
      await expect(pool.connect(lp1).harvestFees(posId, poolId))
        .to.emit(pool, "FeesHarvested")
        .withArgs(posId, lp1.address, 0, 0);
    });
    ...
    it("flashLoan with excessive amount reverts", async function () {
      ...
      await expect(
        pool
          .connect(other)
          .flashLoan(poolId, token0.target, ethers.parseEther("9999"), "0x"),
      ).to.be.reverted;
    });
  ...
  describe("CrossChainRouter — remaining branches", function () {
    ...
    it("registerRelay with insufficient stake reverts", async function () {
      ...
      await expect(
        ccr.connect(relay1).registerRelay({ value: ethers.parseEther("1") }),
      ).to.be.reverted;
    });
    ...
    it("addChain from non-admin reverts", async function () {
      ...
      await expect(
        ccr
          .connect(other)
          .addChain(56, "BSC", 100, 10, 256, 86400, 100, 1000000),
      ).to.be.reverted;
    });
    ...
    it("setTokenSupport from non-admin reverts", async function () {
      ...
      await expect(ccr.connect(other).setTokenSupport(usdc.target, true)).to.be
        .reverted;
    });
  });
});
