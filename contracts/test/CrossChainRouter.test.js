...
describe("CrossChainRouter", function () {
  ...
  async function deployFixture() {
    const [admin, relay1, relay2, sender, treasuryAddr, other] =
  ...
  async function transferInitiatedFixture() {
    ...
    const recipientHash = ethers.keccak256(
      ethers.toUtf8Bytes("recipient_on_polygon"),
    );
  ...
  describe("Deployment", function () {
    ...
    it("should revert with zero admin", async function () {
      ...
      await expect(
        Router.deploy(ethers.ZeroAddress, t.address),
    ...
    it("should revert with zero treasury", async function () {
      ...
      await expect(
        Router.deploy(a.address, ethers.ZeroAddress),
      ).to.be.revertedWithCustomError(Router, "ZeroAddress");
    });
  });
  ...
  describe("Chain Management", function () {
    ...
    it("should revert remove non-existent chain", async function () {
      const { router, admin } = await loadFixture(deployFixture);
      await expect(
        router.connect(admin).removeChain(999),
  ...
  describe("Relay Registration", function () {
    it("should register a relay", async function () {
      ...
      await expect(
        router.connect(relay1).registerRelay({ value: ethers.parseEther("5") }),
    ...
    it("should revert insufficient stake", async function () {
      const { router, relay1 } = await loadFixture(deployFixture);
      await expect(
        router.connect(relay1).registerRelay({ value: ethers.parseEther("1") }),
    ...
    it("should deregister relay by admin", async function () {
      ...
      await expect(
        router.connect(admin).deregisterRelay(relay1.address),
      ).to.emit(router, "RelayDeregistered");
    });
    ...
    it("should revert deregister non-existent relay", async function () {
      ...
      await expect(
        router.connect(other).deregisterRelay(other.address),
  ...
  describe("Transfer Initiation", function () {
    ...
    it("should revert for unsupported token", async function () {
      ...
      await expect(
        router
          .connect(sender)
          .initiateTransfer(other.address, 1000, CHAIN_ID, recipientHash),
    ...
    it("should revert for zero amount", async function () {
      ...
      await expect(
        router
          .connect(sender)
          .initiateTransfer(usdc.target, 0, CHAIN_ID, recipientHash),
  ...
  describe("Relay Operations", function () {
    it("should submit relay proof", async function () {
      ...
      await expect(
        router.connect(relay1).submitRelayProof(transferId, destTxHash, proof),
    ...
    it("should revert relay by non-active relay", async function () {
      ...
      await expect(
        router.connect(other).submitRelayProof(transferId, destTxHash, proof),
    ...
    it("should revert relay with empty proof", async function () {
      ...
      const destTxHash = ethers.keccak256(ethers.toUtf8Bytes("dest_tx_1"));
      await expect(
        router.connect(relay1).submitRelayProof(transferId, destTxHash, "0x"),
      ).to.be.revertedWithCustomError(router, "InvalidProof");
    });

    it("should confirm a relayed transfer", async function () {
      const { router, admin, relay1, transferId } =
        await transferInitiatedFixture();
      const destTxHash = ethers.keccak256(ethers.toUtf8Bytes("dest_tx_1"));
      await router
        .connect(relay1)
        .submitRelayProof(transferId, destTxHash, ethers.toUtf8Bytes("proof"));
      await expect(router.connect(admin).confirmTransfer(transferId)).to.emit(
        router,
        "TransferCompleted",
      );
    });
    ...
    it("should mark transfer as failed", async function () {
      ...
      await router
        ...
        .submitRelayProof(transferId, destTxHash, ethers.toUtf8Bytes("proof"));
      await expect(
        router.connect(admin).markTransferFailed(transferId, "delivery failed"),
  ...
  describe("Recovery", function () {
    ...
    it("should revert recovery before deadline", async function () {
      const { router, sender, transferId } = await transferInitiatedFixture();
      await expect(
        router.connect(sender).recoverTransfer(transferId),
    ...
    it("should recover a failed transfer without waiting", async function () {
      const { router, admin, sender, transferId } =
        await transferInitiatedFixture();
      await router.connect(admin).markTransferFailed(transferId, "failed");
      await expect(router.connect(sender).recoverTransfer(transferId)).to.emit(
        router,
  ...
  describe("View Functions", function () {
    ...
    it("should estimate fee", async function () {
      ...
      expect(estimate.totalFee).to.equal(
        BASE_FEE + (amount * BigInt(FEE_RATE_BP)) / 10000n,
      );
    });

    it("should revert fee estimate for unsupported chain", async function () {
      const { router } = await loadFixture(deployFixture);
      await expect(router.estimateFee(1000, 999)).to.be.revertedWithCustomError(
        router,
        "UnsupportedChain",
      );
    });
  ...
  describe("Admin", function () {
    it("should set token support", async function () {
      ...
      await expect(
        router.connect(admin).setTokenSupport(other.address, true),
      ).to.emit(router, "TokenSupportUpdated");
    });

    it("should update treasury", async function () {
      const { router, admin, other } = await loadFixture(deployFixture);
      await expect(router.connect(admin).setTreasury(other.address)).to.emit(
        router,
        "TreasuryUpdated",
      );
    });
