...
describe("InvoiceFinancing - Deep Coverage", function () {
  ...
  describe("Finance Invoice", function () {
    it("should finance an invoice (partial)", async function () {
      const { invoicing, factor, invoiceId, faceValue } =
        await invoiceFixture();
      const advanceAmount = faceValue / 2n;
      await expect(
        invoicing.connect(factor).financeInvoice(invoiceId, advanceAmount, 300),
    ...
    it("should revert finance with zero advance", async function () {
      ...
      await expect(
        invoicing.connect(factor).financeInvoice(invoiceId, 0, 300),
    ...
    it("should revert finance exceeding face value", async function () {
      ...
      await expect(
        invoicing
          .connect(factor)
          .financeInvoice(invoiceId, faceValue + 1n, 300),
  ...
  describe("Repay Invoice", function () {
    it("should repay invoice directly (no factoring positions)", async function () {
      const { invoicing, debtor, invoiceId, faceValue } =
        await invoiceFixture();
      await expect(
    ...
    it("should repay a financed invoice (pro-rata distribution)", async function () {
      const { invoicing, factor, debtor, invoiceId, faceValue } =
        await invoiceFixture();
      // Finance first
      await invoicing
        .connect(factor)
    ...
    it("should revert repay by non-debtor", async function () {
      ...
      await expect(
        invoicing.connect(other).repayInvoice(invoiceId, faceValue),
    ...
    it("should revert repay zero amount", async function () {
      ...
      await expect(
        invoicing.connect(debtor).repayInvoice(invoiceId, 0),
      ).to.be.revertedWithCustomError(invoicing, "ZeroAmount");
    });

    it("should repay late invoice and update credit score", async function () {
      const { invoicing, debtor, invoiceId, faceValue, maturity } =
        await invoiceFixture();
  ...
  describe("Mark Overdue", function () {
    it("should mark invoice as overdue after grace period", async function () {
      ...
      await expect(invoicing.markOverdue(invoiceId)).to.emit(
        invoicing,
    ...
    it("should revert markOverdue before grace period", async function () {
      ...
      await expect(
        invoicing.markOverdue(invoiceId),
  ...
  describe("Resolve Dispute", function () {
    it("should resolve a dispute with creditor award", async function () {
      ...
      const tx = await invoicing
        .connect(debtor)
        .initiateDispute(invoiceId, "quality issue");
      const receipt = await tx.wait();
      const event = receipt.logs.find(
      ...
      await expect(
        invoicing
          .connect(arbiter)
          .resolveDispute(disputeId, 1, faceValue / 2n, faceValue / 2n),
    ...
    it("should resolve dispute with zero awards", async function () {
      ...
      const tx = await invoicing
        .connect(debtor)
        .initiateDispute(invoiceId, "test");
      const receipt = await tx.wait();
      const event = receipt.logs.find(
        (l) => l.fragment && l.fragment.name === "DisputeInitiated",
      );
      ...
      await expect(
        invoicing.connect(arbiter).resolveDispute(disputeId, 2, 0, 0),
      ).to.emit(invoicing, "DisputeResolved");
    });
  });
  ...
  describe("Deposit Collateral", function () {
    it("should deposit collateral against an invoice", async function () {
      ...
      await expect(
        invoicing
          .connect(debtor)
          .depositCollateral(invoiceId, token.target, amount),
    ...
    it("should revert collateral deposit with zero amount", async function () {
      const { invoicing, token, debtor, invoiceId } = await invoiceFixture();
      await expect(
        invoicing.connect(debtor).depositCollateral(invoiceId, token.target, 0),
      ).to.be.revertedWithCustomError(invoicing, "ZeroAmount");
    });
  });
  ...
  describe("View Functions", function () {
    ...
    it("should return creditor and debtor invoices", async function () {
      ...
      const creditorInvs = await invoicing.getCreditorInvoices(
        creditor.address,
      );
    ...
    it("should return factoring position after financing", async function () {
      ...
      const receipt = await tx.wait();
      const event = receipt.logs.find(
        (l) => l.fragment && l.fragment.name === "InvoiceFinanced",
      );
...
describe("FXHedgingVault - Deep Coverage", function () {
  async function deployFixture() {
    const [admin, hedger, oracle, other, treasuryAddr] =
      await ethers.getSigners();
    const MockERC20 = await ethers.getContractFactory("MockERC20");
  ...
  describe("Emergency Unwind", function () {
    ...
    it("should revert emergency unwind of non-active position", async function () {
      const { vault, admin, oracle, positionId, pairId, maturity } =
        await forwardPositionFixture();
      // Settle first
      ...
      await expect(
        vault.connect(admin).emergencyUnwind(positionId),
  ...
  describe("Maturity too far", function () {
    it("should revert when maturity exceeds MAX_MATURITY", async function () {
      const { vault, collateral, hedger, pairId } =
  ...
  describe("Stale Rate", function () {
    it("should revert settlement with stale rate", async function () {
      const { vault, admin, positionId, maturity } =
        await forwardPositionFixture();
      // Go past maturity AND past rate staleness
      await time.increaseTo(maturity + 86400n * 30n);
      await expect(
        vault.connect(admin).settleForward(positionId),
  ...
  describe("Admin Functions", function () {
    it("should set currency pair active/inactive", async function () {
      ...
      await expect(
        vault.connect(admin).setCurrencyPairActive(pairId, false),
...
describe("ComplianceOracle - Deep Coverage", function () {
  async function deployFixture() {
    const [admin, teeNode1, teeNode2, teeNode3, other] =
      await ethers.getSigners();
  ...
  it("should deregister a non-last TEE node (swap-and-pop)", async function () {
    const { oracle, admin, teeNode1, teeNode2, teeNode3 } =
      await loadFixture(deployFixture);
...
describe("CrossChainRouter - Deep Coverage", function () {
  ...
  it("should deregister non-last relay (swap-and-pop)", async function () {
    const { router, admin, relay1, relay2, relay3 } =
      await loadFixture(deployFixture);
...
describe("LiquidityPool - Deep Coverage", function () {
  async function deployFixture() {
    ...
    const poolTx = await pool
      .connect(admin)
      .createPool(token0.target, token1.target, 30, 10, 7000);
...
describe("TravelRule - Deep Coverage", function () {
  async function deployFixture() {
    ...
    await tr
      .connect(vasp1)
      .registerVASP(ethers.keccak256("0x01"), ethers.toUtf8Bytes("pubkey1"));
    await tr
      .connect(vasp2)
      .registerVASP(ethers.keccak256("0x02"), ethers.toUtf8Bytes("pubkey2"));
...
describe("AIComplianceModule - Deep Coverage", function () {
  async function deployFixture() {
    ...
    await aic
      .connect(admin)
      .grantRole(COMPLIANCE_OFFICER_ROLE, officer.address);

    // Register a model
    const tx = await aic
      .connect(aiOp)
      .registerModel("TestModel", "v1.0", ethers.keccak256("0xaa"));
    const receipt = await tx.wait();
    const event = receipt.logs.find(
  ...
  it("should return override record via getOverride", async function () {
    ...
    // Override it
    const overrideTx = await aic
      .connect(officer)
      .overrideDecision(
        decisionId,
        1,
        ethers.keccak256(ethers.toUtf8Bytes("override_reason")),
      );
    const overrideReceipt = await overrideTx.wait();
    const overrideEvent = overrideReceipt.logs.find(
      (l) => l.fragment && l.fragment.name === "DecisionOverridden",
    );
  ...
  it("should return subject decision count", async function () {
    ...
    await aic
      .connect(aiOp)
      .recordDecision(subjectHash, modelId, 0, 80, evidenceHash, reasonHash);
  ...
  it("should return decision appeal count", async function () {
    ...
    await aic
      .connect(appellant)
      .fileAppeal(decisionId, ethers.keccak256(ethers.toUtf8Bytes("grounds")));
...
describe("NoblePay - Deep Coverage", function () {
  ...
  it("should enforce monthly volume limit", async function () {
    const { noblepay, usdc, admin, business1, recipient } =
      await loadFixture(deployFixture);
...
describe("MultiSigTreasury - Deep Coverage", function () {
  it("should handle fallback receive for native tokens", async function () {
    const [admin, signer1, signer2, signer3, treasury, other] =
      await ethers.getSigners();
