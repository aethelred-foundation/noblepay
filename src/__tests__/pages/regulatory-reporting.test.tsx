...
describe("RegulatoryReportingPage", () => {
  ...
  it("displays jurisdiction filters", () => {
    ...
    expect(screen.getAllByText(/Regulatory Reporting/i).length).toBeGreaterThan(
      0,
    );
  });
  ...
  it("displays KPI stat cards", () => {
    render(<RegulatoryReportingPage />);
    expect(
      screen.getAllByText(/Reports|Filed|Pending/i).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("displays report status badges in table", () => {
    ...
    const allBadges = screen.getAllByText(
      /Filed|Pending|Overdue|Draft|Rejected/,
    );
