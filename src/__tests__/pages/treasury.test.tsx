...
describe("TreasuryPage", () => {
  ...
  it("displays governance section on tab switch", () => {
    ...
    const tabs = screen.getByTestId("tabs");
    const tabButtons = tabs.querySelectorAll("button");
    const governanceTab = Array.from(tabButtons).find(
      (b) =>
        b.textContent?.toLowerCase().includes("governance") ||
        b.textContent?.toLowerCase().includes("proposals"),
    );
    if (governanceTab) {
  ...
  it("clicking proposal items shows details", () => {
    ...
    const voteBtns = buttons.filter(
      (b) => b.textContent?.includes("Vote") || b.textContent?.includes("View"),
    );
  ...
  it("proposals tab shows proposal list with filters", () => {
    render(<TreasuryPage />);
    const tabs = screen.getByTestId("tabs");
    const proposalsBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Proposals"),
    );
    if (proposalsBtn) {
      ...
      const pendingBtn = screen
        .getAllByRole("button")
        .find((b) => b.textContent?.includes("Pending"));
      if (pendingBtn) fireEvent.click(pendingBtn);
      const approvedBtn = screen
        .getAllByRole("button")
        .find((b) => b.textContent?.includes("Approved"));
      if (approvedBtn) fireEvent.click(approvedBtn);
      const executedBtn = screen
        .getAllByRole("button")
        .find((b) => b.textContent?.includes("Executed"));
      if (executedBtn) fireEvent.click(executedBtn);
      const rejectedBtn = screen
        .getAllByRole("button")
        .find((b) => b.textContent?.includes("Rejected"));
      if (rejectedBtn) fireEvent.click(rejectedBtn);
      // Back to all
      const allBtns = screen
        .getAllByRole("button")
        .filter((b) => b.textContent?.startsWith("All"));
  ...
  it("budgets tab shows budget data", () => {
    render(<TreasuryPage />);
    const tabs = screen.getByTestId("tabs");
    const budgetsBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Budgets"),
    );
    if (budgetsBtn) {
  ...
  it("yield tab shows yield strategies", () => {
    render(<TreasuryPage />);
    const tabs = screen.getByTestId("tabs");
    const yieldBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Yield"),
    );
    if (yieldBtn) {
  ...
  it("activity tab shows activity feed", () => {
    render(<TreasuryPage />);
    const tabs = screen.getByTestId("tabs");
    const activityBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
  ...
  it("new proposal modal switches proposal type to Budget Allocation", () => {
    render(<TreasuryPage />);
    fireEvent.click(screen.getByText(/New Proposal/i));
    const budgetBtn = screen
      .getAllByRole("button")
      .find((b) => b.textContent === "Budget Allocation");
    if (budgetBtn) {
  ...
  it("new proposal modal switches proposal type to Yield Strategy", () => {
    render(<TreasuryPage />);
    fireEvent.click(screen.getByText(/New Proposal/i));
    const yieldBtn = screen
      .getAllByRole("button")
      .find((b) => b.textContent === "Yield Strategy");
    if (yieldBtn) {
      fireEvent.click(yieldBtn);
  ...
  it("new proposal modal switches proposal type to Policy Change", () => {
    ...
    const policyBtn = screen
      .getAllByRole("button")
      .find((b) => b.textContent === "Policy Change");
    if (policyBtn) {
  ...
  it("vesting tab shows vesting items with timeUntil", () => {
    render(<TreasuryPage />);
    const tabs = screen.getByTestId("tabs");
    const vestingBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Vesting"),
    );
    if (vestingBtn) {
  ...
  it("budget tab shows budget items with spend percentages", () => {
    render(<TreasuryPage />);
    const tabs = screen.getByTestId("tabs");
    const budgetBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Budget"),
    );
    if (budgetBtn) {
      fireEvent.click(budgetBtn);
      const allText = document.body.textContent || "";
  ...
  it("policy tab shows spending policies", () => {
    render(<TreasuryPage />);
    const tabs = screen.getByTestId("tabs");
    const policyBtn = Array.from(tabs.querySelectorAll("button")).find(
      (b) =>
        b.textContent?.includes("Policy") ||
        b.textContent?.includes("Policies"),
    );
    if (policyBtn) {
      fireEvent.click(policyBtn);
    }
