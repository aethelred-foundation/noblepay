...
describe("RiskMonitorPage", () => {
  ...
  it("alerts tab shows alert list", () => {
    render(<RiskMonitorPage />);
    const tabs = screen.getByTestId("tabs");
    const alertsBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
  ...
  it("counterparties tab shows counterparty data", () => {
    render(<RiskMonitorPage />);
    const tabs = screen.getByTestId("tabs");
    const cpBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Counterpart"),
    );
    if (cpBtn) {
  ...
  it("anomalies tab shows anomaly detection", () => {
    render(<RiskMonitorPage />);
    const tabs = screen.getByTestId("tabs");
    const anomalyBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Anomal"),
    );
    if (anomalyBtn) {
  ...
  it("incidents tab shows incidents list", () => {
    render(<RiskMonitorPage />);
    const tabs = screen.getByTestId("tabs");
    const incBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Incident"),
    );
    if (incBtn) {
  ...
  it("tee tab shows TEE security data", () => {
    render(<RiskMonitorPage />);
    const tabs = screen.getByTestId("tabs");
    const teeBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("TEE"),
    );
    if (teeBtn) {
  ...
  it("charts tab shows analytics charts", () => {
    render(<RiskMonitorPage />);
    const tabs = screen.getByTestId("tabs");
    const chartsBtn = Array.from(tabs.querySelectorAll("button")).find(
      (b) =>
        b.textContent?.includes("Report") || b.textContent?.includes("Chart"),
    );
    if (chartsBtn) {
  ...
  it("renders riskBgColor for all risk score ranges", () => {
    ...
    const bgElements = document.querySelectorAll(
      '[class*="bg-emerald"], [class*="bg-amber"], [class*="bg-red"], [class*="bg-orange"]',
    );
  ...
  it("alert filter buttons change filter state", () => {
    ...
    const tabs = screen.getByTestId("tabs");
    const alertsTab = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Alert"),
    );
    if (alertsTab) {
      ...
      const filterBtns = screen
        .getAllByRole("button")
        .filter((b) =>
          ["All", "Critical", "High", "Medium", "Low"].includes(
            b.textContent || "",
          ),
        );
      filterBtns.forEach((btn) => fireEvent.click(btn));
  ...
  it("alerts tab filter by High severity works", () => {
    ...
    if (alertsTab) {
      ...
      const highBtn = screen
        .getAllByRole("button")
        .find((b) => b.textContent?.startsWith("High"));
      if (highBtn) fireEvent.click(highBtn);
      // Click Medium filter
      const medBtn = screen
        .getAllByRole("button")
        .find((b) => b.textContent?.startsWith("Medium"));
      if (medBtn) fireEvent.click(medBtn);
      // Click Low filter
      const lowBtn = screen
        .getAllByRole("button")
        .find((b) => b.textContent?.startsWith("Low"));
