...
describe("StreamingPage", () => {
  ...
  it("clicking stream card opens detail", () => {
    ...
    const viewBtns = cards.filter(
      (b) =>
        b.textContent?.includes("View") || b.textContent?.includes("Details"),
    );
  ...
  it("stream cards show action buttons for active streams", () => {
    ...
    const pauseBtns = screen
      .getAllByRole("button")
      .filter((b) => b.textContent?.includes("Pause"));
    expect(pauseBtns.length).toBeGreaterThan(0);
    const adjustBtns = screen
      .getAllByRole("button")
      .filter((b) => b.textContent?.includes("Adjust Rate"));
  ...
  it("clicking Pause button on active stream stops propagation", () => {
    ...
    const pauseActionBtns = allBtns.filter(
      (b) =>
        b.className.includes("bg-amber-500") &&
  ...
  it("clicking Adjust Rate button on active stream stops propagation", () => {
    render(<StreamingPage />);
    const allBtns = screen.getAllByRole("button");
    const adjustBtns = allBtns.filter(
      (b) =>
        b.className.includes("bg-blue-500") &&
        b.textContent?.includes("Adjust Rate"),
    );
    expect(adjustBtns.length).toBeGreaterThan(0);
  ...
  it("clicking Cancel button on active stream stops propagation", () => {
    ...
    const allBtns = screen.getAllByRole("button");
    const cancelActionBtns = allBtns.filter(
      (b) =>
  ...
  it("paused streams have Resume button", () => {
    ...
    const pausedBtn = screen
      .getAllByRole("button")
      .find((b) => b.textContent === "Paused");
    if (pausedBtn) {
      fireEvent.click(pausedBtn);
      const resumeBtns = screen
        .getAllByRole("button")
  ...
  it("clicking a stream card opens detail drawer", () => {
    ...
    if (allText.length > 0) {
      const card =
        allText[0].closest('[class*="cursor"]') ||
        allText[0].closest("div[onClick]");
  ...
  it("calendar tab shows upcoming milestones", () => {
    render(<StreamingPage />);
    const tabs = screen.getByTestId("tabs");
    const calendarBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
  ...
  it("history tab shows stream history table", () => {
    render(<StreamingPage />);
    const tabs = screen.getByTestId("tabs");
    const historyBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
  ...
  it("analytics tab shows streaming velocity chart", () => {
    render(<StreamingPage />);
    const tabs = screen.getByTestId("tabs");
    const analyticsBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
  ...
  it("batch payroll modal has upload area and manual entry", () => {
    ...
    expect(screen.getByText("Batch Payroll Creation")).toBeInTheDocument();
    expect(
      screen.getByText("Upload CSV file with payroll data"),
    ).toBeInTheDocument();
  ...
  it("create stream modal submit closes modal", () => {
    ...
    const createBtns = screen
      .getAllByRole("button")
      .filter((b) => b.textContent === "Create Stream");
  ...
  it("batch payroll modal submit closes modal", () => {
    ...
    const submitBtns = screen
      .getAllByRole("button")
      .filter((b) => b.textContent === "Create Batch Streams");
  ...
  it("displays stream status badges", () => {
    ...
    const badges = screen.getAllByText(
      /Active|Paused|Completed|Scheduled|Cancelled/,
    );
  ...
  it("paused stream cards show Resume and Cancel buttons", () => {
    ...
    const cancelBtns = screen.queryAllByText(/Cancel/);
    const pausedCancelBtns = cancelBtns.filter(
      (b) => b.closest("button") !== pausedFilter,
    );
  ...
  it("history tab renders action styles for all action types", () => {
    ...
    if (historyBtn) {
      ...
      const allActions = screen.getAllByText(
        /Created|Paused|Resumed|Rate Adjusted|Cancelled|Completed|Cliff Released/,
      );
