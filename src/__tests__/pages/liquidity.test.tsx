...
describe("LiquidityPage", () => {
  ...
  it("switches to all tabs and back", () => {
    ...
    // Go back to pools
    const poolsBtn = Array.from(tabButtons).find((b) =>
      b.textContent?.includes("Pools"),
    );
  ...
  it("positions tab shows position data", () => {
    ...
    const tabs = screen.getByTestId("tabs");
    const tabButtons = tabs.querySelectorAll("button");
    const positionsBtn = Array.from(tabButtons).find((b) =>
      b.textContent?.includes("Positions"),
    );
    if (positionsBtn) {
  ...
  it("analytics tab shows charts", () => {
    ...
    const tabs = screen.getByTestId("tabs");
    const tabButtons = tabs.querySelectorAll("button");
    const analyticsBtn = Array.from(tabButtons).find((b) =>
  ...
  it("rewards tab shows staking data", () => {
    ...
    const tabs = screen.getByTestId("tabs");
    const tabButtons = tabs.querySelectorAll("button");
    const rewardsBtn = Array.from(tabButtons).find((b) =>
      b.textContent?.includes("Rewards"),
    );
    if (rewardsBtn) {
  ...
  it("flash tab shows flash liquidity data", () => {
    ...
    const tabs = screen.getByTestId("tabs");
    const tabButtons = tabs.querySelectorAll("button");
    const flashBtn = Array.from(tabButtons).find((b) =>
      b.textContent?.includes("Flash"),
    );
    if (flashBtn) {
  ...
  it("clicking a position card opens position detail", () => {
    render(<LiquidityPage />);
    const tabs = screen.getByTestId("tabs");
    const positionsBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Positions"),
    );
    if (positionsBtn) {
      fireEvent.click(positionsBtn);
  ...
  it("add liquidity modal has form fields", () => {
    ...
    expect(
      screen.getByText("Price Range (Concentrated Liquidity)"),
    ).toBeInTheDocument();
  });
  ...
  it("add liquidity modal submit button works", () => {
    ...
    const addBtns = screen
      .getAllByRole("button")
      .filter((b) => b.textContent === "Add Liquidity");
