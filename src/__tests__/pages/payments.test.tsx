...
describe("PaymentsPage", () => {
  ...
  it("clicking a payment row opens the detail drawer", () => {
    ...
    const viewBtns = rows.filter(
      (b) =>
        b.textContent?.includes("View") ||
        b.getAttribute("title")?.includes("View"),
    );
  ...
  it("pagination controls navigate pages", () => {
    ...
    const nextBtn = allBtns.find(
      (b) =>
        b.textContent?.includes("Next") ||
        b.querySelector("svg")?.closest("button") === b,
    );
    if (nextBtn) {
  ...
  it("currency filter changes displayed payments", () => {
    render(<PaymentsPage />);
    const usdcPills = screen
      .getAllByRole("button")
      .filter((b) => b.textContent === "USDC");
  ...
  it("risk level filter applies correctly", () => {
    render(<PaymentsPage />);
    const highPills = screen
      .getAllByRole("button")
      .filter((b) => b.textContent === "High");
  ...
  it("sort by status works", () => {
    ...
    const statusSortable = statusHeaders.find(
  ...
  it("sort by risk works", () => {
    ...
    const riskHeader = screen
      .getAllByRole("button")
      .find((b) => b.textContent?.includes("Risk"));
    if (riskHeader) {
  ...
  it("sort by settlement works", () => {
    render(<PaymentsPage />);
    const settlementHeader = screen
      .getAllByRole("button")
      .find((b) => b.textContent?.includes("Settlement"));
    if (settlementHeader) {
  ...
  it("sort by status toggles direction on second click", () => {
    ...
    const statusBtns = screen
      .getAllByRole("button")
      .filter((b) => b.textContent?.includes("Status"));
  ...
  it("new payment modal currency select works", () => {
    ...
    const currencySelect = screen
      ...
      .find((s) => (s as HTMLSelectElement).value === "USDC");
    if (currencySelect) {
  ...
  it("new payment modal close button works", () => {
    ...
    const closeBtn = closeBtns.find(
      (b) => b.closest(".fixed") && !b.textContent,
    );
  ...
  it("payment detail drawer shows compliance timeline", () => {
    ...
    fireEvent.click(viewBtns[0]);
    expect(
      screen.getByText("Compliance Screening Timeline"),
    ).toBeInTheDocument();
  ...
  it("bulk upload button is present", () => {
    ...
    const bulkBtn = screen
      .getAllByRole("button")
      .find((b) => b.textContent?.includes("Bulk Upload"));
  ...
  it("date range filter 30d button works", () => {
    ...
    const btn = screen
      .getAllByRole("button")
      .find((b) => b.textContent === "30d");
    if (btn) fireEvent.click(btn);
  });

  it("sort by Recipient works", () => {
    render(<PaymentsPage />);
    const recipientBtns = screen
      .getAllByRole("button")
      .filter((b) => b.textContent?.includes("Recipient"));
  ...
  it("payment detail drawer shows copy buttons that work", () => {
    ...
    if (drawerEl) {
      ...
      const copyBtns = btnsInDrawer.filter((b) =>
        b.className.includes("p-0.5"),
      );
  ...
  it("pagination shows end page range when near last page", () => {
    ...
    if (page5) {
      ...
      if (highPages.length > 0)
        fireEvent.click(highPages[highPages.length - 1]);
    }
  });
  ...
  it("opens detail drawer for a Settled payment and shows Initiate Refund button", () => {
    ...
    const settledPill = screen
      .getAllByRole("button")
      .find((b) => b.textContent === "Settled");
    if (settledPill) {
      fireEvent.click(settledPill);
      const viewBtns = screen.queryAllByTitle("View details");
      if (viewBtns.length > 0) {
