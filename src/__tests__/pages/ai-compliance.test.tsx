...
describe("AICompliancePage", () => {
  ...
  it("displays KPI stat cards", () => {
    render(<AICompliancePage />);
    expect(
      screen.getAllByText(/Accuracy|Precision|Decisions/i).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("filter buttons change active state", () => {
    ...
    const filterBtns = buttons.filter((b) =>
      ["All", "Approved", "Flagged", "Blocked", "Review"].includes(
        b.textContent || "",
      ),
    );
  ...
  it("models tab shows model registry", () => {
    render(<AICompliancePage />);
    const tabs = screen.getByTestId("tabs");
    const modelsBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
  ...
  it("review tab shows review queue", () => {
    render(<AICompliancePage />);
    const tabs = screen.getByTestId("tabs");
    const reviewBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
  ...
  it("performance tab shows metrics charts", () => {
    render(<AICompliancePage />);
    const tabs = screen.getByTestId("tabs");
    const perfBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Performance"),
    );
    if (perfBtn) {
      fireEvent.click(perfBtn);
      expect(
        screen.getByText("Precision / Recall Over Time"),
      ).toBeInTheDocument();
    }
  });
  ...
  it("bias tab shows bias monitoring data", () => {
    render(<AICompliancePage />);
    const tabs = screen.getByTestId("tabs");
    const biasBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Bias"),
    );
    if (biasBtn) {
  ...
  it("appeals tab shows appeals data", () => {
    render(<AICompliancePage />);
    const tabs = screen.getByTestId("tabs");
    const appealsBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Appeal"),
    );
    if (appealsBtn) {
  ...
  it("heatmap tab shows risk heatmap", () => {
    render(<AICompliancePage />);
    const tabs = screen.getByTestId("tabs");
    const heatmapBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Risk Map"),
    );
    if (heatmapBtn) {
  ...
  it("drawer shows decision details with amount and confidence", () => {
    ...
    if (cards.length > 0) {
      ...
      if (drawer) {
        ...
        expect(
          screen.getByText("Decision Factor Analysis"),
        ).toBeInTheDocument();
      }
    }
  });
  ...
  it("jurisdiction risk map uses fallback for unknown jurisdictions", () => {
    ...
    const tabs = screen.getByTestId("tabs");
    const riskMapBtn = Array.from(tabs.querySelectorAll("button")).find((b) =>
      b.textContent?.includes("Risk Map"),
    );
    if (riskMapBtn) {
