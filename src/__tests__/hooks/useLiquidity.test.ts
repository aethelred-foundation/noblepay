...
describe("useLiquidity", () => {
  ...
  it("analytics has correct computed values", () => {
    ...
    expect(analytics.totalTvl).toBe(
      8_500_000 + 15_200_000 + 2_100_000 + 5_600_000,
    );
    expect(analytics.totalVolume24h).toBe(
      1_200_000 + 3_400_000 + 450_000 + 890_000,
    );
  ...
  it("addLiquidity increases pool TVL and reserves", () => {
    ...
    const updated = result.current.pools.find(
      (p) => p.address === "0xpool001",
    )!;
  ...
  it("addLiquidity does not affect other pools", () => {
    ...
    const pool2Tvl = result.current.pools.find(
      (p) => p.address === "0xpool002",
    )!.tvl;
    ...
    expect(
      result.current.pools.find((p) => p.address === "0xpool002")!.tvl,
    ).toBe(pool2Tvl);
  });
  ...
  it("removeLiquidity removes a position", () => {
    ...
    expect(
      result.current.positions.find((p) => p.id === "pos-001"),
    ).toBeUndefined();
    expect(
      result.current.positions.find((p) => p.id === "pos-002"),
    ).toBeDefined();
  });
  ...
  it("claimFees does not affect other positions", () => {
    ...
    const pos2Fees = result.current.positions.find(
      (p) => p.id === "pos-002",
    )!.unclaimedFees;
    ...
    expect(
      result.current.positions.find((p) => p.id === "pos-002")!.unclaimedFees,
    ).toBe(pos2Fees);
  });
