...
const DEBTOR_NAMES = [
  ...
  "Sterling Commerce Ltd",
  "Vanguard Exports",
];

const DISPUTE_REASONS = [
  "Invoice amount discrepancy",
  "Goods not received as specified",
  "Duplicate invoice submitted",
  "Incorrect payment terms",
  "Quality dispute — partial delivery",
...
function generateInvoices(count: number): Invoice[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    invoices.push({
      ...
      debtorName:
        DEBTOR_NAMES[Math.floor(seededRandom(seed + 40) * DEBTOR_NAMES.length)],
      amount,
      funded:
        status === "Funded" || status === "Settled"
          ? amount * (seededRandom(seed + 50) * 0.1 + 0.88)
          : 0,
      discountRate: seededRandom(seed + 60) * 4 + 1.5,
      issueDate: Date.now() - daysOffset * 86400000,
      dueDate:
        Date.now() + Math.floor(seededRandom(seed + 70) * 90 - 30) * 86400000,
      status,
      daysBucket,
    });
  }
...
function generateDisputes(): Dispute[] {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    disputes.push({
      ...
      reason:
        DISPUTE_REASONS[
          Math.floor(seededRandom(seed + 10) * DISPUTE_REASONS.length)
        ],
      filedBy:
        DEBTOR_NAMES[Math.floor(seededRandom(seed + 20) * DEBTOR_NAMES.length)],
...
function generateAgingData(): Array<{
  bucket: string;
  count: number;
  value: number;
  fill: string;
...
function CreditGradeDisplay({
  ...
  return (
    <div className="text-center">
      <div
        className={`text-5xl font-black ${GRADE_COLORS[grade] || "text-slate-400"}`}
      >
        {grade}
      </div>
...
export default function InvoiceFinancingPage() {
  ...
  const totalOutstanding = useMemo(
    () => invoices.reduce((s, inv) => s + inv.amount, 0),
    [invoices],
  );
  const totalFunded = useMemo(
    () =>
      invoices
        ...
        .reduce((s, inv) => s + inv.funded, 0),
    [invoices],
  );
  const avgDiscount = useMemo(
    () =>
      invoices.reduce((s, inv) => s + inv.discountRate, 0) / invoices.length,
    [invoices],
  );
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Enterprise receivables factoring, credit scoring, and dispute
                  management
                </p>
              </div>
              <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <GlassCard className="p-4" hover={false}>
              <div className="flex items-start justify-between">
                ...
                <Sparkline
                  data={generateSparklineData(100, 12)}
                  color={BRAND.red}
                  height={28}
                  width={64}
                />
            ...
            <GlassCard className="p-4" hover={false}>
              <div className="flex items-start justify-between">
                ...
                <Sparkline
                  data={generateSparklineData(200, 12)}
                  color="#10B981"
                  height={28}
                  width={64}
                />
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            ...
                            <div className="text-xs text-slate-500 font-mono">
                              {truncateAddress(inv.debtor, 8, 4)}
                            </div>
                          </td>
