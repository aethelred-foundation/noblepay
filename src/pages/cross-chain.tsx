...
function generateTransfers(count: number): Transfer[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const status =
      statuses[Math.floor(seededRandom(seed + 20) * statuses.length)];
    const stepIdx =
      status === "Completed"
        ? 3
        : status === "Failed"
    ...
    transfers.push({
      ...
      amount:
        i === 0 ? 500 : Math.floor(seededRandom(seed + 40) * 500000) + 1000,
...
function NodeStatusBadge({ status }: { status: RelayNode["status"] }) {
  ...
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full bg-current ${status !== "Online" ? "animate-pulse" : ""}`}
      />
      {status}
    </span>
  );
  ...
  return (
    <div className="flex items-center gap-1">
      {TRANSFER_STEPS.map((step, i) => {
        const isComplete =
          i < currentIdx || (i === currentIdx && status === "Completed");
        ...
        return (
          <div key={step} className="flex items-center gap-1">
            ...
            {i < TRANSFER_STEPS.length - 1 && (
              <div
                className={`w-4 h-0.5 ${i < currentIdx ? "bg-emerald-500/50" : "bg-slate-700/50"}`}
              />
            )}
          </div>
        );
...
export default function CrossChainPage() {
  ...
  const avgSettlement = useMemo(() => {
    ...
    return (
      completed.reduce((s, t) => s + t.estimatedTime, 0) / completed.length
    );
  }, [transfers]);
  const activeRelays = useMemo(
    () => relayNodes.filter((n) => n.status === "Online").length,
    [relayNodes],
  );
  ...
  return (
    <>
        ...
        <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Multi-chain asset bridging, relay monitoring, and route
                  optimization
                </p>
              </div>
              <button
            ...
            {chainStatuses.map((chain) => (
              <GlassCard key={chain.id} className="p-4" hover>
                ...
                <div className="flex items-center gap-1.5 mb-3">
                  <span
                    className={`w-2 h-2 rounded-full ${chain.active ? "bg-emerald-400 animate-pulse" : "bg-red-400"}`}
                  />
                  <span
                    className={`text-xs font-medium ${chain.active ? "text-emerald-400" : "text-red-400"}`}
                  >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span
                      className={`font-medium ${chain.avgLatency < 300 ? "text-emerald-400" : chain.avgLatency < 600 ? "text-amber-400" : "text-red-400"}`}
                    >
                      {chain.avgLatency}ms
                    </span>
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            <GlassCard className="p-4" hover={false}>
              <div className="flex items-start justify-between">
                ...
                <Sparkline
                  data={generateSparklineData(300, 12)}
                  color={BRAND.red}
                  height={28}
                  width={64}
                />
            ...
            <GlassCard className="p-4" hover={false}>
              <div className="flex items-start justify-between">
                ...
                <Sparkline
                  data={generateSparklineData(400, 12)}
                  color="#10B981"
                  height={28}
                  width={64}
                />
            ...
            <GlassCard className="p-4" hover={false}>
              ...
              <p className="text-xl font-bold text-white">
                {activeRelays}/{relayNodes.length}
              </p>
              <div className="text-xs text-slate-500 mt-1">
                {((activeRelays / relayNodes.length) * 100).toFixed(0)}% online
              </div>
            </GlassCard>
            <GlassCard className="p-4" hover={false}>
              <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-xs text-slate-500 mt-1">
                {completedTransfers.length} completed today
              </div>
            </GlassCard>
          </div>
                    ...
                    >
                      <td className="px-4 py-3">
                        ...
                        <div className="text-[10px] text-slate-500 font-mono mt-0.5">
                          {truncateAddress(t.txHash, 10, 6)}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5 text-xs">
                          <span className="text-white">
                            {chainIcon(t.sourceChain)}{" "}
                            {chainName(t.sourceChain)}
                          </span>
                          <ArrowRight className="w-3 h-3 text-slate-500" />
                          <span className="text-white">
                            {chainIcon(t.destChain)} {chainName(t.destChain)}
                          </span>
                        </div>
                        <div className="text-[10px] text-slate-500 font-mono mt-0.5">
                          {truncateAddress(t.sender, 6, 4)} →{" "}
                          {truncateAddress(t.recipient, 6, 4)}
                        </div>
                      </td>
                      <td className="px-4 py-3">
          ...
          <GlassCard className="p-0 overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-700/50">
              ...
              <span className="text-xs text-slate-400">
                {activeRelays} of {relayNodes.length} online
              </span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                  ...
                  {relayNodes.map((node) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-slate-400">
                        {truncateAddress(node.address, 8, 4)}
                      </td>
                      <td className="px-4 py-3 text-right text-white font-medium">
                        {formatUSD(node.stake)}
                      </td>
                      <td className="px-4 py-3 text-right">
                        <span
                          className={`font-medium ${node.successRate >= 99 ? "text-emerald-400" : node.successRate >= 97 ? "text-amber-400" : "text-red-400"}`}
                        >
                          {node.successRate.toFixed(2)}%
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right text-slate-300">
                        {node.totalRelayed.toLocaleString()}
                      </td>
                      <td className="px-4 py-3">
      ...
      <Modal
        ...
        title="New Cross-Chain Transfer"
        maxWidth="max-w-lg"
      >
          ...
          <div className="rounded-lg bg-slate-800/30 border border-slate-700/30 p-3">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <span className="text-white font-medium">
                {chainIcon(sourceChain)} {chainName(sourceChain)}
              </span>
              <ChevronRight className="w-3 h-3" />
              <span className="text-amber-400 font-medium">Relay Pool</span>
              <ChevronRight className="w-3 h-3" />
              <span className="text-white font-medium">
                {chainIcon(destChain)} {chainName(destChain)}
              </span>
            </div>
