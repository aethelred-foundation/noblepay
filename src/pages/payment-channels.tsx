...
const COUNTERPARTY_NAMES = [
  ...
  "Gulf Settlement Network",
  "Nordic Payment Hub",
  "Atlas Financial Services",
  "Pinnacle Clearing",
  "Sterling Payments Ltd",
  "Horizon Networks",
  "Crescent Gateway",
  "Vanguard Payment Co.",
];
...
function generateChannels(count: number): Channel[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    channels.push({
      ...
      counterpartyName:
        COUNTERPARTY_NAMES[
          Math.floor(seededRandom(seed + 30) * COUNTERPARTY_NAMES.length)
        ],
      capacity,
      ...
      openedAt:
        Date.now() - Math.floor(seededRandom(seed + 60) * 90 * 86400000),
...
function generateThroughputData(): Array<{
  hour: string;
  throughput: number;
  settlements: number;
}> {
...
function ChannelStatusBadge({ status }: { status: ChannelStatus }) {
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${CHANNEL_STATUS_STYLES[status] || ""}`}
    >
...
export default function PaymentChannelsPage() {
  ...
  const totalCapacity = useMemo(
    () => channels.reduce((s, c) => s + c.capacity, 0),
    [channels],
  );
  const totalLocalBalance = useMemo(
    () => channels.reduce((s, c) => s + c.localBalance, 0),
    [channels],
  );
  const totalTxCount = useMemo(
    () => channels.reduce((s, c) => s + c.txCount, 0),
    [channels],
  );
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Off-chain payment routing, channel management, and settlement
                  analytics
                </p>
              </div>
              <button
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-xs text-slate-500 font-mono ml-2">
                          {truncateAddress(ch.counterparty, 8, 4)}
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-slate-400">
                        {ch.txCount.toLocaleString()} txns
                      </span>
                    ...
                    {settlements.slice(0, 10).map((s) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5">
                          <span
                            className={`text-xs font-medium ${s.direction === "Inbound" ? "text-emerald-400" : "text-amber-400"}`}
                          >
                            {s.direction}
                          </span>
                        </td>
