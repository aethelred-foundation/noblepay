...
const COMPANY_NAMES = [
  ...
  "Falcon Fintech Solutions",
  "Desert Rose Trading",
  "Gulf Stream Finance",
  "Phoenix Partners AG",
  "Oasis Digital Assets",
  "Atlas Venture Holdings",
  "Zenith Corporation",
  "Crescent Bay Ventures",
  "Sovereign Wealth Partners",
  "Noble Bridge Capital",
  "Apex Advisory Group",
  "Titanium Holdings",
  ...
  return [
    {
      ...
      score: 18,
      trend: "down",
      description: "OFAC/UN/EU sanctions list proximity risk",
    },
    {
      name: "AML Risk Score",
      key: "aml",
    ...
    },
    {
      ...
      score: 24,
      trend: "up",
      description: "Aggregate jurisdiction risk across corridors",
    },
    {
      ...
      score: 15,
      trend: "down",
      description: "Transaction behavior pattern analysis",
    },
    {
      ...
      score: 21,
      trend: "stable",
      description: "Payment network graph risk assessment",
    },
    {
      name: "Volume Anomaly",
      key: "volume",
      score: 28,
      trend: "up",
      description: "Transaction volume deviation from baseline",
    },
    {
      ...
      score: 72,
      trend: "up",
      description: "Elevated risk from high-risk payment corridors",
    },
    {
      ...
      score: 88,
      trend: "stable",
      description: "ML-based fraud detection scoring",
    },
  ];
}
...
function generateAlerts(count: number): TransactionAlert[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const severity =
      severities[Math.floor(seededRandom(seed) * severities.length)];
    const status =
      statuses[Math.floor(seededRandom(seed + 10) * statuses.length)];
    alerts.push({
      ...
      status,
      type: ALERT_TYPES[
        Math.floor(seededRandom(seed + 30) * ALERT_TYPES.length)
      ],
      message: `${ALERT_TYPES[Math.floor(seededRandom(seed + 30) * ALERT_TYPES.length)]} detected for transaction.`,
      riskScore:
        severity === "Critical"
          ? 75 + Math.floor(seededRandom(seed + 40) * 25)
          : severity === "High"
            ? 50 + Math.floor(seededRandom(seed + 40) * 25)
            : severity === "Medium"
              ? 25 + Math.floor(seededRandom(seed + 40) * 25)
              : Math.floor(seededRandom(seed + 40) * 25),
      amount:
        i === 0 ? 500 : Math.floor(seededRandom(seed + 50) * 500000) + 5000,
      currency:
        currencies[Math.floor(seededRandom(seed + 60) * currencies.length)],
      sender: seededAddress(seed + 70),
      senderName:
        COMPANY_NAMES[
          Math.floor(seededRandom(seed + 80) * COMPANY_NAMES.length)
        ],
      recipient: seededAddress(seed + 90),
      recipientName:
        COMPANY_NAMES[
          Math.floor(seededRandom(seed + 100) * COMPANY_NAMES.length)
        ],
      jurisdiction:
        JURISDICTIONS[
          Math.floor(seededRandom(seed + 110) * JURISDICTIONS.length)
        ],
      timestamp: Date.now() - Math.floor(seededRandom(seed + 120) * 604800000),
      acknowledgedBy:
        status === "Acknowledged" || status === "Resolved"
          ? REVIEWER_NAMES[
              Math.floor(seededRandom(seed + 130) * REVIEWER_NAMES.length)
            ]
          : null,
      resolvedAt:
        status === "Resolved"
          ? Date.now() - Math.floor(seededRandom(seed + 140) * 172800000)
          : null,
    });
  }
...
function generateCounterparties(): CounterpartyProfile[] {
  ...
  const riskLevels: CounterpartyProfile["riskLevel"][] = [
    "Low",
    "Low",
    "Medium",
    "High",
  ...
  const tiers: CounterpartyProfile["tier"][] = [
    "Standard",
    "Premium",
    "Enterprise",
  ];
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const riskLevel =
      riskLevels[Math.floor(seededRandom(seed) * riskLevels.length)];
    profiles.push({
      address: seededAddress(seed + 10),
      name: COMPANY_NAMES[i % COMPANY_NAMES.length],
      jurisdiction:
        JURISDICTIONS[
          Math.floor(seededRandom(seed + 20) * JURISDICTIONS.length)
        ],
      riskScore:
        riskLevel === "Low"
          ? Math.floor(seededRandom(seed + 30) * 25)
          : riskLevel === "Medium"
            ? 25 + Math.floor(seededRandom(seed + 30) * 25)
            : riskLevel === "High"
              ? 50 + Math.floor(seededRandom(seed + 30) * 25)
      ...
      lastActivity:
        Date.now() - Math.floor(seededRandom(seed + 70) * 2592000000),
...
function generateAnomalies(): AnomalyAlert[] {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    anomalies.push({
      ...
      description:
        descriptions[Math.floor(seededRandom(seed + 10) * descriptions.length)],
      confidence: 60 + seededRandom(seed + 20) * 35,
      relatedEntities: [
        COMPANY_NAMES[
          Math.floor(seededRandom(seed + 30) * COMPANY_NAMES.length)
        ],
        COMPANY_NAMES[
          Math.floor(seededRandom(seed + 40) * COMPANY_NAMES.length)
        ],
      ],
      timestamp: Date.now() - Math.floor(seededRandom(seed + 50) * 604800000),
      severity: (["Medium", "High", "Critical"] as const)[
        Math.floor(seededRandom(seed + 60) * 3)
      ],
    });
  }
...
function generateIncidents(): Incident[] {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    incidents.push({
      ...
      status,
      severity: (["Medium", "High", "Critical"] as const)[
        Math.floor(seededRandom(seed + 10) * 3)
      ],
      category:
        INCIDENT_CATEGORIES[
          Math.floor(seededRandom(seed + 20) * INCIDENT_CATEGORIES.length)
        ],
      createdBy: seededAddress(seed + 30),
      createdByName:
        REVIEWER_NAMES[
          Math.floor(seededRandom(seed + 40) * REVIEWER_NAMES.length)
        ],
      ...
      assignedToName:
        REVIEWER_NAMES[
          Math.floor(seededRandom(seed + 90) * REVIEWER_NAMES.length)
        ],
    });
  }
  return incidents;
  ...
  return [
    {
      ...
      lastAttestation: Date.now() - 120000,
      uptime: 99.97,
      processedCount: 487231,
      avgLatencyMs: 38,
      errorRate: 0.01,
      version: "v2.4.1",
    },
    {
      ...
      status: "Healthy",
      attestationValid: true,
      lastAttestation: Date.now() - 180000,
      uptime: 99.94,
      processedCount: 412890,
      avgLatencyMs: 41,
      errorRate: 0.02,
      version: "v2.4.1",
    },
    {
      ...
      status: "Healthy",
      attestationValid: true,
      lastAttestation: Date.now() - 90000,
      uptime: 99.99,
      processedCount: 298456,
      avgLatencyMs: 35,
      errorRate: 0.005,
      version: "v2.4.1",
    },
    ...
    },
    {
      ...
      lastAttestation: Date.now() - 7200000,
      uptime: 0,
      processedCount: 89234,
      avgLatencyMs: 0,
      errorRate: 100,
      version: "v2.4.0",
    },
...
function generateRiskScoreChart(): Array<{
  date: string;
  overall: number;
  sanctions: number;
  aml: number;
...
function generateAlertFrequency(): Array<{
  day: string;
  low: number;
  medium: number;
  high: number;
...
function generateResolutionTimes(): Array<{
  category: string;
  avgHours: number;
  fill: string;
}> {
...
function SeverityBadge({ severity }: { severity: AlertSeverity }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${styles[severity]}`}
    >
      {severity}
    </span>
  );
...
function AlertStatusBadge({ status }: { status: AlertStatus }) {
  ...
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${status === "Active" ? "bg-red-400 animate-pulse" : status === "Acknowledged" ? "bg-amber-400" : status === "Resolved" ? "bg-emerald-400" : "bg-slate-400"}`}
      />
      {status}
    </span>
  );
...
function TEEStatusBadge({ status }: { status: TEEStatus }) {
  ...
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${status === "Healthy" ? "bg-emerald-400 animate-pulse" : status === "Degraded" ? "bg-amber-400" : "bg-red-400"}`}
      />
      {status}
    </span>
  );
...
export default function RiskMonitorPage() {
  ...
  const [selectedAlert, setSelectedAlert] = useState<TransactionAlert | null>(
    null,
  );
  ...
  const overallRiskScore = useMemo(
    () =>
      Math.round(
        riskFactors.reduce((s, f) => s + f.score, 0) / riskFactors.length,
      ),
    [riskFactors],
  );
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#0f172a] text-slate-100">
        ...
        <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Enterprise risk command center with real-time transaction
                  monitoring and TEE health
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
              ...
              <StatCard
                ...
                change={{
                  value: `${alerts.filter((a) => a.severity === "Critical").length} critical`,
                  positive: false,
                }}
              />
              <StatCard
                ...
                change={{
                  value: `${incidents.filter((i) => i.status === "Investigating").length} investigating`,
                  positive: false,
                }}
              />
              <StatCard
              ...
              <StatCard
                ...
                change={{
                  value: `${teeNodes.filter((n) => n.status === "Offline").length} offline`,
                  positive: healthyNodes === teeNodes.length,
                }}
              />
              <StatCard
                icon={Clock}
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <GlassCard className="p-6">
                  <SectionHeader
                    title="Risk Radar"
                    subtitle="System-wide risk profile"
                    size="sm"
                  />
                  {mounted && (
                ...
                <GlassCard className="lg:col-span-2 p-6">
                  <SectionHeader
                    ...
                    subtitle="Contributing factors to system risk"
                    size="sm"
                  />
                    ...
                    {riskFactors.map((factor) => (
                      <div key={factor.key} className="flex items-center gap-4">
                        <div
                          ...
                          style={{
                            backgroundColor: RISK_FACTOR_COLORS[factor.key],
                          }}
                        />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <div className="flex items-center gap-2">
                              <span
                                className={`text-sm font-semibold tabular-nums ${riskColor(factor.score)}`}
                              >
                                {factor.score}/100
                              </span>
                              {factor.trend === "up" && (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-400">
                            {alert.senderName} &middot;{" "}
                            {formatUSD(alert.amount)} &middot;{" "}
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-2 text-xs text-slate-400">
                          <span>
                            {alert.senderName} &rarr; {alert.recipientName}
                          </span>
                          <span>&middot;</span>
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <div className="mt-1">
                          <span
                            className={`text-xs font-medium tabular-nums ${riskColor(alert.riskScore)}`}
                          >
          ...
          {activeTab === "counterparties" && (
            <GlassCard className="p-6">
              <SectionHeader
                title="Counterparty Risk Profiles"
                subtitle="Entity risk assessment and transaction history"
                size="sm"
              />
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-slate-500">
                                  {cp.jurisdiction} &middot; {cp.tier}
                                </p>
                              </div>
                            </div>
                          </td>
                          <td className="py-3 px-3 text-center">
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${riskStyles[cp.riskLevel]}`}
                            >
                              {cp.riskLevel}
                            </span>
                          </td>
                          <td className="py-3 px-3 text-right">
                            <span
                              className={`text-sm font-semibold tabular-nums ${riskColor(cp.riskScore)}`}
                            >
                              {cp.riskScore}
                            </span>
                          </td>
                          <td className="py-3 px-3 text-right text-sm text-white tabular-nums">
                            {formatUSD(cp.totalVolume)}
                          </td>
                          <td className="py-3 px-3 text-right text-sm text-slate-300 tabular-nums">
                            {formatNumber(cp.totalTransactions)}
                          </td>
                          <td className="py-3 px-3 text-right">
                            <span
                              className={`text-sm tabular-nums ${cp.flaggedTransactions > 10 ? "text-red-400" : "text-slate-300"}`}
                            >
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs text-slate-400">
                          ...
                          <span>&middot;</span>
                          <span>
                            Related: {anomaly.relatedEntities.join(", ")}
                          </span>
                          <span>&middot;</span>
                          <span>{timeAgo(anomaly.timestamp)}</span>
                        </div>
                ...
                <GlassCard key={node.id} className="p-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4">
                    <div className="p-3 rounded-xl bg-slate-800/40">
                      ...
                      <p
                        className={`text-sm font-semibold ${node.attestationValid ? "text-emerald-400" : "text-red-400"}`}
                      >
                    ...
                    <div className="p-3 rounded-xl bg-slate-800/40">
                      ...
                      <p className="text-sm font-semibold text-white">
                        {timeAgo(node.lastAttestation)}
                      </p>
                    </div>
                    <div className="p-3 rounded-xl bg-slate-800/40">
                      <p className="text-xs text-slate-400">Uptime</p>
                      <p
                        className={`text-sm font-semibold ${node.uptime > 99.9 ? "text-emerald-400" : node.uptime > 99 ? "text-amber-400" : "text-red-400"}`}
                      >
                    ...
                    <div className="p-3 rounded-xl bg-slate-800/40">
                      ...
                      <p className="text-sm font-semibold text-white">
                        {formatNumber(node.processedCount)}
                      </p>
                    </div>
                    <div className="p-3 rounded-xl bg-slate-800/40">
                    ...
                    <div className="p-3 rounded-xl bg-slate-800/40">
                      ...
                      <p
                        className={`text-sm font-semibold ${node.errorRate < 0.1 ? "text-emerald-400" : node.errorRate < 1 ? "text-amber-400" : "text-red-400"}`}
                      >
                        {node.errorRate < 1
                          ? `${(node.errorRate * 100).toFixed(0)}bps`
            ...
            <div className="space-y-8">
              ...
              <GlassCard className="p-6">
                <SectionHeader
                  ...
                  subtitle="Weekly risk score by category (last 12 weeks)"
                  size="sm"
                />
                {mounted && (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <GlassCard className="p-6">
                  <SectionHeader
                    ...
                    subtitle="Alerts by severity (this week)"
                    size="sm"
                  />
                  {mounted && (
                ...
                <GlassCard className="p-6">
                  <SectionHeader
                    ...
                    subtitle="By alert category (hours)"
                    size="sm"
                  />
                  {mounted && (
                    <ResponsiveContainer width="100%" height={280}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/30">
                ...
                <p className="text-lg font-bold text-white">
                  {formatUSD(selectedAlert.amount)}
                </p>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/30">
                <p className="text-xs text-slate-400">Risk Score</p>
                <p
                  className={`text-lg font-bold ${riskColor(selectedAlert.riskScore)}`}
                >
              ...
              <div className="flex items-center gap-3 mb-3">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-500 font-mono">
                    {truncateAddress(selectedAlert.sender, 10, 6)}
                  </p>
                </div>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm font-medium text-white">
                    {selectedAlert.recipientName}
                  </p>
                  <p className="text-xs text-slate-500 font-mono">
                    {truncateAddress(selectedAlert.recipient, 10, 6)}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-mono text-slate-300">
                  {truncateAddress(selectedAlert.paymentId, 10, 6)}
                </span>
              </div>
              <div className="flex justify-between text-xs">
      ...
      <Modal
        ...
        title="Create Compliance Incident"
        maxWidth="max-w-2xl"
      >
        <div className="space-y-4">
          <div>
