...
const BUSINESS_NAMES = [
  "Al Maktoum Trading LLC",
  "Emirates Global Finance",
  "Dubai Silicon Oasis Tech FZ-LLC",
  "Abu Dhabi Investment Partners",
  "Sharjah Digital Services",
  "RAK International Holdings",
  "Ajman Commercial Group",
  "Fujairah Maritime Logistics",
  "Al Ain Agritech Solutions",
  "DIFC Capital Management",
  "Jebel Ali Free Zone Corp",
  "Meydan Group Holdings",
  "Al Barsha Technologies",
  "Deira Gold Trading LLC",
  "Business Bay Consulting FZ",
  "Palm Jumeirah Ventures",
  "Al Quoz Industrial LLC",
  "Maritime City Services FZ-LLC",
  "Dubai Healthcare Partners",
  "Abu Dhabi Energy Corp",
  "Sharjah Media Group LLC",
  "RAK Ceramics Trading",
  "Ajman Port Logistics",
  "Fujairah Oil Terminal LLC",
  "Al Reem Island Finance",
  "Saadiyat Investment Group",
  "Yas Marina Trading LLC",
  "Dubai Internet City Tech",
  "Masdar City Green Energy",
  "Khalifa Port Services LLC",
];
...
const TIER_CONFIG: Record<
  BusinessTier,
  { label: string; color: string; bg: string; ring: string; dailyLimit: number }
> = {
  STANDARD: {
    label: "Standard",
...
function generateBusinesses(): Business[] {
  ...
  return Array.from({ length: 30 }, (_, i) => {
    ...
    const licenseNum = String(
      Math.floor(seededRandom(seed) * 9000000 + 1000000),
    );
...
function generateVerificationQueue(): VerificationItem[] {
  const docStatuses: VerificationItem["documentsStatus"][] = [
    "Complete",
    "Partial",
    "Complete",
    "Missing",
  ...
  return [
    {
      businessName: "Al Khaleej Trading FZ-LLC",
      submittedDate: "2024-01-12",
      documentsStatus: docStatuses[0],
      licenseNo: "DAFZA-8834521",
    },
    {
      businessName: "Gulf Stream Logistics",
      submittedDate: "2024-01-11",
      documentsStatus: docStatuses[1],
      licenseNo: "DED-2024-112233",
    },
    {
      businessName: "Reem Island Tech Solutions",
      submittedDate: "2024-01-10",
      documentsStatus: docStatuses[2],
      licenseNo: "AFZA-7712345",
    },
    {
      businessName: "Marina Walk Investments",
      submittedDate: "2024-01-09",
      documentsStatus: docStatuses[3],
      licenseNo: "CN-5567890",
    },
    {
      businessName: "Corniche Capital Partners",
      submittedDate: "2024-01-08",
      documentsStatus: docStatuses[4],
      licenseNo: "SPC-3345678",
    },
  ];
}
...
function generateReverificationAlerts(): ReverificationAlert[] {
  return [
    {
      businessName: "Al Maktoum Trading LLC",
      lastVerified: "2023-01-10",
      daysRemaining: -5,
      category: "overdue",
    },
    {
      businessName: "RAK International Holdings",
      lastVerified: "2023-01-20",
      daysRemaining: -1,
      category: "overdue",
    },
    {
      businessName: "Dubai Silicon Oasis Tech FZ-LLC",
      lastVerified: "2023-02-01",
      daysRemaining: 8,
      category: "this_month",
    },
    {
      businessName: "DIFC Capital Management",
      lastVerified: "2023-02-05",
      daysRemaining: 12,
      category: "this_month",
    },
    {
      businessName: "Emirates Global Finance",
      lastVerified: "2023-02-15",
      daysRemaining: 22,
      category: "this_month",
    },
    {
      businessName: "Abu Dhabi Investment Partners",
      lastVerified: "2023-03-01",
      daysRemaining: 36,
      category: "next_month",
    },
    {
      businessName: "Sharjah Digital Services",
...
function TierBadge({ tier }: { tier: BusinessTier }) {
  const config = TIER_CONFIG[tier];
  const icons: Record<BusinessTier, typeof Star> = {
    STANDARD: Shield,
    PREMIUM: Star,
    ENTERPRISE: Award,
  };
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${config.bg} ${config.color} ${config.ring}`}
    >
...
function KYCBadge({ status }: { status: KYCStatus }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ring-white/10 ${s.bg} ${s.text}`}
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${s.dot} ${status === "Verified" ? "animate-pulse" : ""}`}
      />
      {status}
    </span>
  );
...
function StatCard({
  ...
  const toneMap: Record<string, { card: string; icon: string }> = {
    green: {
      card: "from-emerald-500/10 to-emerald-900/10 border-emerald-700/30",
      icon: "text-emerald-400",
    },
    red: {
      card: "from-red-500/10 to-red-900/10 border-red-700/30",
      icon: "text-red-400",
    },
    amber: {
      card: "from-amber-500/10 to-amber-900/10 border-amber-700/30",
      icon: "text-amber-400",
    },
    blue: {
      card: "from-blue-500/10 to-blue-900/10 border-blue-700/30",
      icon: "text-blue-400",
    },
  };
...
export default function BusinessRegistryPage() {
  ...
  const reverificationAlerts = useMemo(
    () => generateReverificationAlerts(),
    [],
  );
  ...
  const filteredBusinesses = useMemo(() => {
    return businesses.filter((b) => {
      ...
      if (jurisdictionFilter !== "all" && b.jurisdiction !== jurisdictionFilter)
        return false;
      if (searchQuery) {
  ...
  const thisMonth = reverificationAlerts.filter(
    (a) => a.category === "this_month",
  );
  const nextMonth = reverificationAlerts.filter(
    (a) => a.category === "next_month",
  );
      ...
      <div className="min-h-screen bg-[#0f172a] text-slate-100">
        ...
        <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">
                UAE business onboarding, KYC verification, and tier management
              </p>
            </div>
            <button
          ...
          <SectionHeader
            ...
            subtitle="Business tiers and corresponding transaction limits"
            size="sm"
          />
          ...
          <SectionHeader
            ...
            subtitle="All registered UAE businesses"
            size="sm"
          />
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-700/50">
                    ...
                    <th className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wide text-slate-400">
                      Tier
                    </th>
                    <th className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wide text-slate-400">
                      KYC Status
                ...
                <tbody>
                  {paginatedBusinesses.map((biz) => {
                    const scoreColor =
                      biz.complianceScore >= 95
                        ? "#10B981"
                        : biz.complianceScore >= 85
                          ? "#F59E0B"
                          : "#DC2626";
                    return (
                      <tr
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          <KYCBadge status={biz.kycStatus} />
                        </td>
                        <td className="px-4 py-3 text-right text-xs text-white tabular-nums">
                          ${formatNumber(biz.dailyVolume)}
                        </td>
                        <td className="px-4 py-3">
                  ...
                  {verificationQueue.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          <span
                            className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${docColor[item.documentsStatus]}`}
                          >
          ...
          {/* ================================================================
              ...
              ================================================================ */}
          <SectionHeader
            title="Re-verification Alerts"
            subtitle="Annual KYC re-verification schedule"
            size="sm"
          />
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-10">
            ...
            <GlassCard className="p-5 border-red-700/30" hover={false}>
              ...
              <div className="space-y-3">
                {overdue.map((a) => (
                  <div
                    key={a.businessName}
                    className="rounded-lg bg-red-500/5 border border-red-800/30 p-3"
                  >
            ...
            <GlassCard className="p-5 border-amber-700/30" hover={false}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="ml-auto rounded-full bg-amber-500/10 px-2 py-0.5 text-xs font-bold text-amber-400">
                  {thisMonth.length}
                </span>
              </div>
              <div className="space-y-3">
                {thisMonth.map((a) => (
                  <div
                    key={a.businessName}
                    className="rounded-lg bg-amber-500/5 border border-amber-800/30 p-3"
                  >
                    <p className="text-xs font-medium text-white">
                      {a.businessName}
                    </p>
            ...
            <GlassCard className="p-5 border-blue-700/30" hover={false}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="ml-auto rounded-full bg-blue-500/10 px-2 py-0.5 text-xs font-bold text-blue-400">
                  {nextMonth.length}
                </span>
              </div>
              <div className="space-y-3">
                {nextMonth.map((a) => (
                  <div
                    key={a.businessName}
                    className="rounded-lg bg-blue-500/5 border border-blue-800/30 p-3"
                  >
                    <p className="text-xs font-medium text-white">
                      {a.businessName}
                    </p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={label} className="rounded-lg bg-slate-800/50 p-3">
                      ...
                      <p className="mt-0.5 text-xs text-white font-medium truncate">
                        {val}
                      </p>
                    </div>
                  ))}
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between rounded-lg bg-slate-800/50 p-3">
                    ...
                    <span className="text-xs text-white">
                      {selectedBusiness.nextVerification}
                    </span>
                  </div>
                  <div className="rounded-lg bg-slate-800/50 p-3">
                    ...
                    <p className="font-mono text-[10px] text-slate-300 mt-1 break-all">
                      {selectedBusiness.complianceOfficer}
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="rounded-lg bg-slate-800/50 p-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-white tabular-nums">
                        ${formatNumber(selectedBusiness.usedToday)} / $
                        {formatNumber(selectedBusiness.dailyLimit)}
                      </span>
                    </div>
                    <MiniProgressBar
                      value={selectedBusiness.usedToday}
                      max={selectedBusiness.dailyLimit}
                      color="#3B82F6"
                    />
                  </div>
                  <div className="rounded-lg bg-slate-800/50 p-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-white tabular-nums">
                        ${formatNumber(selectedBusiness.usedMonth)} / $
                        {formatNumber(selectedBusiness.monthlyLimit)}
                      </span>
                    </div>
                    <MiniProgressBar
                      value={selectedBusiness.usedMonth}
                      max={selectedBusiness.monthlyLimit}
                      color="#F59E0B"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="rounded-lg bg-amber-500/5 border border-amber-800/30 p-3 text-center">
                    <p className="text-lg font-bold text-amber-400">
                      {Math.round(
                        seededRandom(
                          parseInt(selectedBusiness.id.replace("BIZ-", "")) +
                            100,
                        ) * 3,
                      )}
                    </p>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {Array.from({ length: 10 }, (_, i) => {
                    const seed =
                      parseInt(selectedBusiness.id.replace("BIZ-", "")) * 100 +
                      i * 7;
                    const amount = Math.round(
                      seededRandom(seed) * selectedBusiness.dailyLimit * 0.3,
                    );
                    const status =
                      seededRandom(seed + 1) > 0.15 ? "Success" : "Pending";
                    return (
                      <div
                        key={i}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`h-1.5 w-1.5 rounded-full ${status === "Success" ? "bg-emerald-400" : "bg-amber-400"}`}
                          />
                        </div>
                      </div>
                    );
