...
function generateDecisions(count: number): ComplianceDecision[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let f = 0; f < factorCount; f++) {
      ...
      factors.push({
        name: FACTOR_NAMES[
          Math.floor(seededRandom(fSeed) * FACTOR_NAMES.length)
        ],
    ...
    decisions.push({
      ...
      confidence:
        outcome === "Approved"
          ? 85 + seededRandom(seed + 40) * 15
          : 40 + seededRandom(seed + 40) * 50,
      modelId: models[Math.floor(seededRandom(seed + 50) * models.length)],
      modelVersion: "v3.2.1",
      riskScore:
        outcome === "Approved"
          ? Math.floor(seededRandom(seed + 60) * 30)
          : outcome === "Flagged"
            ? 40 + Math.floor(seededRandom(seed + 60) * 30)
            : outcome === "Blocked"
              ? 70 + Math.floor(seededRandom(seed + 60) * 30)
              : 30 + Math.floor(seededRandom(seed + 60) * 40),
      factors,
      jurisdiction: JURISDICTIONS[jIdx].code,
      amount:
        i === 0 ? 500 : Math.floor(seededRandom(seed + 70) * 500000) + 5000,
      currency:
        currencies[Math.floor(seededRandom(seed + 80) * currencies.length)],
      timestamp: Date.now() - Math.floor(seededRandom(seed + 90) * 604800000),
      processingTimeMs: Math.floor(seededRandom(seed + 100) * 80) + 15,
      senderName:
        COMPANY_NAMES[
          Math.floor(seededRandom(seed + 110) * COMPANY_NAMES.length)
        ],
      recipientName:
        COMPANY_NAMES[
          Math.floor(seededRandom(seed + 120) * COMPANY_NAMES.length)
        ],
    });
  }
...
function generateReviewQueue(): HumanReviewItem[] {
  ...
  const priorities: HumanReviewItem["priority"][] = [
    "Low",
    "Medium",
    "High",
    "Critical",
  ];
  const statuses: HumanReviewItem["status"][] = [
    "Pending",
    "In Progress",
    "Completed",
  ];
  ...
  const decisions = allDecisions.filter(
    (d) => d.outcome === "Review" || d.outcome === "Flagged",
  );
  ...
  for (let i = 0; i < 8; i++) {
    ...
    items.push({
      ...
      assignedName:
        REVIEWER_NAMES[
          Math.floor(seededRandom(seed + 20) * REVIEWER_NAMES.length)
        ],
      priority:
        priorities[Math.floor(seededRandom(seed + 30) * priorities.length)],
...
function generateAppeals(): Appeal[] {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    appeals.push({
      ...
      submittedByName:
        COMPANY_NAMES[
          Math.floor(seededRandom(seed + 50) * COMPANY_NAMES.length)
        ],
      submittedAt:
        Date.now() - Math.floor(seededRandom(seed + 60) * 1209600000),
      resolvedAt:
        status === "Resolved" || status === "Rejected"
          ? Date.now() - Math.floor(seededRandom(seed + 70) * 604800000)
          : null,
      resolution:
        status === "Resolved"
          ...
          : status === "Rejected"
            ...
            : null,
      originalOutcome: (["Flagged", "Blocked"] as const)[
        Math.floor(seededRandom(seed + 80) * 2)
      ],
      amount: Math.floor(seededRandom(seed + 90) * 300000) + 10000,
      currency: ["USDC", "USDT", "AETHEL"][
        Math.floor(seededRandom(seed + 100) * 3)
      ],
    });
  }
  return appeals;
    ...
    return {
      ...
      flagRate:
        risk === "Low"
          ...
          : risk === "Medium"
            ? 3 + seededRandom(seed + 10) * 8
            : risk === "High"
              ? 10 + seededRandom(seed + 10) * 15
...
function generateBiasMetrics(): BiasMetric[] {
  return [
    {
      category: "Low-Risk Jurisdictions",
      detectionRate: 2.3,
    ...
    {
      category: "Medium-Risk Jurisdictions",
      detectionRate: 8.7,
      falsePositiveRate: 3.4,
      avgConfidence: 91.5,
      sampleSize: 180000,
    ...
    {
      category: "Small Transactions (<$10K)",
      detectionRate: 1.8,
    ...
    {
      category: "Large Transactions (>$100K)",
      detectionRate: 12.3,
...
function generatePrecisionRecallChart(): Array<{
  month: string;
  precision: number;
  recall: number;
  f1: number;
...
function generateFPRateChart(): Array<{
  week: string;
  sentinel: number;
  sanctions: number;
  behavioral: number;
...
function OutcomeBadge({ outcome }: { outcome: DecisionOutcome }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${styles[outcome]}`}
    >
      {outcome}
    </span>
  );
...
function ModelStatusBadge({ status }: { status: AIModel["status"] }) {
  ...
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${status === "Production" ? "bg-emerald-400 animate-pulse" : status === "Staging" ? "bg-amber-400" : "bg-slate-400"}`}
      />
      {status}
    </span>
  );
...
export default function AICompliancePage() {
  ...
  const [outcomeFilter, setOutcomeFilter] = useState<"all" | DecisionOutcome>(
    "all",
  );
  ...
  const avgAccuracy =
    productionModels.reduce((s, m) => s + m.accuracy, 0) /
  ...
  const openAppeals = appeals.filter(
    (a) => a.status === "Open" || a.status === "Under Review",
  ).length;
        ...
        <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  AI-powered compliance decision intelligence, model monitoring,
                  and bias analysis
                </p>
              </div>
          ...
          {activeTab === "decisions" && (
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-2">
                {(
                  ["all", "Approved", "Flagged", "Blocked", "Review"] as const
                ).map((filter) => (
                  <button
                    key={filter}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 mt-2 text-xs text-slate-400">
                          <span>
                            Confidence:{" "}
                            <span
                              className={riskColor(100 - decision.confidence)}
                            >
                          ...
                          <span>
                            Risk:{" "}
                            <span className={riskColor(decision.riskScore)}>
                              {decision.riskScore}
                            </span>
                          </span>
                ...
                <GlassCard key={model.id} className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        {model.id} &middot; {model.version} &middot; Updated{" "}
                        {timeAgo(model.lastUpdated)}
                      </p>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4">
                    ...
                    <div className="p-3 rounded-xl bg-slate-800/40">
                      ...
                      <p className="text-sm font-semibold text-amber-400">
                        {model.falsePositiveRate.toFixed(1)}%
                      </p>
                    </div>
                    <div className="p-3 rounded-xl bg-slate-800/40">
                      <p className="text-xs text-slate-400">FN Rate</p>
                      <p className="text-sm font-semibold text-red-400">
                        {model.falseNegativeRate.toFixed(1)}%
                      </p>
                    </div>
                    <div className="p-3 rounded-xl bg-slate-800/40">
                      <p className="text-xs text-slate-400">Decisions</p>
                      <p className="text-sm font-semibold text-white">
                        {formatNumber(model.totalDecisions)}
                      </p>
                    </div>
                    <div className="p-3 rounded-xl bg-slate-800/40">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-2 text-xs text-slate-400">
                        ...
                        <span>&middot;</span>
                        <span>
                          Confidence: {item.decision.confidence.toFixed(0)}%
                        </span>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="text-sm font-semibold text-white tabular-nums">
                        {formatUSD(item.decision.amount)}
                      </p>
                      {item.status !== "Completed" && (
          ...
          {activeTab === "heatmap" && (
            <div className="space-y-6">
              <GlassCard className="p-6">
                <SectionHeader
                  title="Jurisdiction Risk Heat Map"
                  subtitle="AI detection rates and confidence by region"
                  size="sm"
                />
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {jurisdictionRisk.map((j) => {
                    ...
                    return (
                      <div
                        key={j.code}
                        className={`p-4 rounded-xl border ${riskStyles[j.risk]}`}
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span
                            className={`text-xs font-medium ${riskTextStyles[j.risk]}`}
                          >
                            {j.risk}
                          </span>
                        </div>
                        ...
                        <div className="mt-2 space-y-1">
                          <div className="flex justify-between text-xs">
                            ...
                            <span className={riskTextStyles[j.risk]}>
                              {j.flagRate.toFixed(1)}%
                            </span>
                          </div>
                          <div className="flex justify-between text-xs">
                            <span className="text-slate-500">Confidence</span>
                            <span className="text-slate-300">
                              {j.avgConfidence.toFixed(1)}%
                            </span>
                          </div>
                          <div className="flex justify-between text-xs">
                            <span className="text-slate-500">Decisions</span>
                            <span className="text-slate-300">
                              {formatNumber(j.decisions)}
                            </span>
                          </div>
                        </div>
                      </div>
          ...
          {activeTab === "performance" && (
            <div className="space-y-8">
              ...
              <GlassCard className="p-6">
                <SectionHeader
                  title="Precision / Recall Over Time"
                  subtitle="Model performance metrics (last 9 months)"
                  size="sm"
                />
                {mounted && (
              ...
              <GlassCard className="p-6">
                <SectionHeader
                  ...
                  subtitle="Weekly FP rate trend (last 12 weeks)"
                  size="sm"
                />
                {mounted && (
                  <ResponsiveContainer width="100%" height={300}>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {biasMetrics.map((metric) => {
                      const biasRisk =
                        metric.falsePositiveRate > 5
                          ? "High"
                          : metric.falsePositiveRate > 3
                            ? "Medium"
                            : "Low";
                      const biasStyles = {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-3 px-3 text-right text-sm text-white tabular-nums">
                            {metric.detectionRate.toFixed(1)}%
                          </td>
                          <td className="py-3 px-3 text-right">
                            <span
                              className={`text-sm font-medium tabular-nums ${metric.falsePositiveRate > 4 ? "text-red-400" : metric.falsePositiveRate > 2 ? "text-amber-400" : "text-emerald-400"}`}
                            >
                          ...
                          <td className="py-3 px-3 text-center">
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${biasStyles[biasRisk]}`}
                            >
                              {biasRisk}
                            </span>
                          </td>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${statusStyles[appeal.status]}`}
                          >
                            {appeal.status}
                          </span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/30">
                ...
                <p className="text-lg font-bold text-white">
                  {formatUSD(selectedDecision.amount)}
                </p>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/30">
                <p className="text-xs text-slate-400">Confidence</p>
                <p className="text-lg font-bold text-white">
                  {selectedDecision.confidence.toFixed(1)}%
                </p>
              </div>
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/30">
                <p className="text-xs text-slate-400">Risk Score</p>
                <p
                  className={`text-lg font-bold ${riskColor(selectedDecision.riskScore)}`}
                >
              ...
              <div className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/30">
                ...
                <p className="text-lg font-bold text-white">
                  {selectedDecision.processingTimeMs}ms
                </p>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-800/40 border border-slate-700/30">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-white">
                    {selectedDecision.recipientName}
                  </p>
                </div>
              </div>
            </div>
