...
function generateAuditEntries(baseSeed: number, count: number): AuditEntry[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const eventType =
      EVENT_TYPES[Math.floor(seededRandom(s) * EVENT_TYPES.length)];
    const descriptions = EVENT_DESCRIPTIONS[eventType];
    const severity =
      SEVERITIES[Math.floor(seededRandom(s + 1) * SEVERITIES.length)];
    ...
    entries.push({
      ...
      description:
        descriptions[Math.floor(seededRandom(s + 8) * descriptions.length)],
      severity,
...
function generateSystemEvents(baseSeed: number): SystemEvent[] {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const type =
      SYSTEM_EVENT_TYPES[
        Math.floor(seededRandom(s) * SYSTEM_EVENT_TYPES.length)
      ];
    const severity: Severity =
      seededRandom(s + 1) > 0.85
        ? "Critical"
        : seededRandom(s + 1) > 0.7
          ? "Warning"
          : "Info";
...
function generateAuditBySeverity(baseSeed: number) {
  return [
    {
      name: "Info",
      value: Math.floor(9000 + seededRandom(baseSeed) * 2000),
      color: "#3B82F6",
    },
    {
      name: "Warning",
      value: Math.floor(500 + seededRandom(baseSeed + 1) * 800),
      color: "#F59E0B",
    },
    {
      name: "Critical",
      value: Math.floor(50 + seededRandom(baseSeed + 2) * 150),
      color: "#EF4444",
    },
  ];
}
      ...
      >
        ...
        <td className="py-3 pr-3">
          <span className="font-mono text-xs text-slate-300">
            {truncateAddress(entry.eventId, 8, 6)}
          </span>
        </td>
        <td className="py-3 pr-3 hidden sm:table-cell">
          <span
            className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${EVENT_TYPE_COLORS[entry.eventType]}`}
          >
            {entry.eventType}
          </span>
        </td>
        <td className="py-3 pr-3 hidden md:table-cell">
          <span className="font-mono text-xs text-slate-400">
            {entry.actor === "system"
              ? "SYSTEM"
              : truncateAddress(entry.actor, 6, 4)}
          </span>
        </td>
        <td className="py-3 pr-3 text-sm text-slate-200 hidden lg:table-cell">
          {entry.description}
        </td>
        <td className="py-3 pr-3 hidden xl:table-cell">
          <span
            className={`inline-flex items-center gap-1 text-xs font-medium ${SEVERITY_COLORS[entry.severity]}`}
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${SEVERITY_DOT[entry.severity]}`}
            />
            {entry.severity}
          </span>
        </td>
        <td className="py-3 pr-3 text-xs text-slate-400 hidden xl:table-cell">
    ...
    <>
      ...
      <div className="min-h-screen bg-[#0f172a] text-slate-100">
        ...
        <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <SectionHeader
            title="Audit Log"
            subtitle="Expandable entries with cryptographic hash chain"
            size="sm"
          />
          ...
          <SectionHeader
            ...
            action={
              ...
              >
                ...
                />
                {verifying
                  ? `Verifying (${verifiedCount}/${chainEntries.length})`
                  : "Verify Chain"}
              </button>
            }
          />
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className={`text-xs font-medium ${EVENT_TYPE_COLORS[entry.eventType]} px-2 py-0.5 rounded-full`}
                          >
                            {entry.eventType}
                          </span>
                          <span className="text-xs text-slate-500">
                        ...
                        <div className="flex flex-wrap gap-x-6 gap-y-1">
                          <span className="text-xs text-slate-400">
                            ...
                            <span className="font-mono text-slate-300">
                              {truncateAddress(entry.eventId, 10, 8)}
                            </span>
                          </span>
                          <span className="text-xs text-slate-400">
                            Prev:{" "}
                            <span className="font-mono text-slate-500">
                              {truncateAddress(entry.prevHash, 10, 8)}
                            </span>
                          </span>
                        </div>
                      </div>
          ...
          <SectionHeader
            ...
            subtitle="Download compliance reports for regulators and auditors"
            size="sm"
          />
          ...
          <SectionHeader
            title="System Events"
            subtitle="TEE node lifecycle, sanctions updates, and configuration changes"
            size="sm"
          />
                  ...
                  >
                    ...
                    <td className="py-3 pr-4 hidden md:table-cell">
                      <span className="font-mono text-xs text-slate-400">
                        {event.actor === "system"
                          ? "SYSTEM"
                          : truncateAddress(event.actor, 6, 4)}
                      </span>
                    </td>
                    <td className="py-3">
                      <span
                        className={`inline-flex items-center gap-1 text-xs font-medium ${SEVERITY_COLORS[event.severity]}`}
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${SEVERITY_DOT[event.severity]}`}
                        />
                        {event.severity}
                      </span>
                    </td>
          ...
          <SectionHeader
            ...
            subtitle="Event distribution and trends"
            size="sm"
          />
