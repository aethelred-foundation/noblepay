...
const REPORT_TEMPLATES = [
  {
    ...
    description:
      "Report suspicious transactions exceeding threshold limits for AML compliance.",
  ...
  },
  {
    ...
    description:
      "Mandatory filing for currency transactions above regulatory thresholds.",
  ...
  },
  {
    ...
    description:
      "Transaction monitoring report for potentially suspicious activities.",
  ...
  },
  {
    ...
    description:
      "Cross-border wire transfer reporting as per FATF Recommendation 16.",
...
function generateReports(count: number): GeneratedReport[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const template =
      REPORT_TEMPLATES[
        Math.floor(seededRandom(seed) * REPORT_TEMPLATES.length)
      ];
    const jurisdiction =
      template.jurisdictions[
        Math.floor(seededRandom(seed + 10) * template.jurisdictions.length)
      ];
    const status =
    ...
    reports.push({
      ...
      status,
      createdAt:
        Date.now() - Math.floor(seededRandom(seed + 30) * 30 * 86400000),
      dueDate:
        Date.now() +
        Math.floor((seededRandom(seed + 40) - 0.3) * 14 * 86400000),
      filedAt:
        status === "Filed"
          ? Date.now() - Math.floor(seededRandom(seed + 50) * 7 * 86400000)
          : undefined,
      referenceId: `REF-${Math.floor(seededRandom(seed + 60) * 999999)
        .toString()
        .padStart(6, "0")}`,
      txCount: Math.floor(seededRandom(seed + 70) * 50) + 1,
      totalAmount:
        i === 0 ? 500 : Math.floor(seededRandom(seed + 80) * 5000000) + 10000,
    });
  }
...
function generateDeadlines(): Deadline[] {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    deadlines.push({
      ...
      reportType:
        REPORT_TEMPLATES[
          Math.floor(seededRandom(seed + 10) * REPORT_TEMPLATES.length)
        ].code,
      jurisdiction:
        JURISDICTIONS[
...
function ReportStatusBadge({ status }: { status: ReportStatus }) {
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${REPORT_STATUS_STYLES[status] || ""}`}
    >
...
export default function RegulatoryReportingPage() {
  ...
  const pendingCount = useMemo(
    () =>
      reports.filter((r) => r.status === "Pending" || r.status === "Draft")
        .length,
    [reports],
  );
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Compliance filing, deadline tracking, and multi-jurisdiction
                  reporting
                </p>
              </div>
            </div>
              ...
              {REPORT_TEMPLATES.map((tmpl) => {
                ...
                return (
                  <GlassCard key={tmpl.code} className="p-4" hover>
                    <div
                      className={`inline-flex p-2 rounded-lg border mb-3 ${tmpl.color}`}
                    >
                    ...
                    <div className="flex flex-wrap gap-1 mt-3">
                      {tmpl.jurisdictions.map((j) => (
                        <span
                          key={j}
                          className="px-1.5 py-0.5 rounded text-xs bg-slate-700/50 text-slate-300"
                        >
                          {j}
                        </span>
                      ))}
                ...
                <tbody>
                  {filteredReports.slice(0, 12).map((r) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span className="px-1.5 py-0.5 rounded text-xs bg-slate-700/50 text-slate-300">
                          {r.jurisdiction}
                        </span>
                      </td>
