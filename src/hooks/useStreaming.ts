...
export function useStreaming(userAddress?: string) {
  ...
  const [balances, setBalances] = useState<Map<string, StreamBalance>>(
    new Map(),
  );
    ...
    const calculateBalances = () => {
      ...
      for (const stream of streams) {
        ...
        const streamed = Math.min(
          stream.ratePerSecond * elapsed,
          stream.totalAmount,
        );
        const withdrawn =
          stream.streamedAmount - (streamed - stream.streamedAmount);
        ...
        newBalances.set(stream.id, {
          ...
          withdrawable: Math.max(
            ...
            streamed -
              ...
              (stream.totalAmount - stream.totalAmount),
          ),
          remaining: Math.max(0, stream.totalAmount - streamed),
          deposited: stream.totalAmount,
          withdrawn:
            stream.totalAmount - stream.totalAmount + stream.streamedAmount,
          snapshotAt: now,
        });
      }
  ...
  useEffect(() => {
    ...
    setAnalytics({
      ...
      totalRemainingValue: streams.reduce(
        (sum, s) => sum + (s.totalAmount - s.streamedAmount),
        0,
      ),
